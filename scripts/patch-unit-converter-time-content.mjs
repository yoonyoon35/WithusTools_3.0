/**
 * toolContent.json에 Time Converter 허브 + 전용 페어 페이지 + UI 병합
 */
...
import {
  ...
  timeUiEn,
  timeUiKo,
} from "./unit-converter-time-ui-data.mjs";

const root = process.cwd();
const HUB_PATH = "tools.unit-converter.time";

function withShared(uiMap, toolUi) {
  return { ...uiMap.shared, ...toolUi, shared: uiMap.shared };
}

function getCanonicalTimeSlug(fromKey, toKey) {
...
function buildPairContent(fromKey, toKey, uiMap, locale) {
  ...
  const faqA2 =
    locale === "ko"
      ? "예. 수식 줄, 설명 섹션, 시간 변환 표가 포함되어 있습니다."
      : "Yes. This page includes formula lines, explanation sections, and time conversion tables.";
  const faqQ3 =
    locale === "ko"
      ? "다른 시간 단위 쌍 페이지로 이동할 수 있나요?"
      : "Can I move to other time pair pages?";
  const faqA3 =
    locale === "ko"
      ? "예. 아래 관련 시간 단위 쌍 링크에서 이동할 수 있습니다."
      : "Yes. Cross-links to related time unit pairs are provided below.";
...
for (const locale of ["en", "ko"]) {
  ...
  const uiMap = locale === "en" ? timeUiEn : timeUiKo;
  const hubSrc = locale === "en" ? timeHubContentEn : timeHubContentKo;
  ...
  console.log(`Patched ${locale}/toolContent.json: time hub + ${pairCount} pair pages`);
}
