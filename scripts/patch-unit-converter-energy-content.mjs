/**
 * toolContent.json에 Energy Converter 허브 + 전용 페어 페이지 + UI 병합
 */
...
import {
  ...
  energyUiEn,
  energyUiKo,
} from "./unit-converter-energy-ui-data.mjs";

const root = process.cwd();
const HUB_PATH = "tools.unit-converter.energy";

function withShared(uiMap, toolUi) {
  return { ...uiMap.shared, ...toolUi, shared: uiMap.shared };
}

function getCanonicalEnergySlug(fromKey, toKey) {
...
function buildPairUi(uiMap) {
  return withShared(uiMap, {
    ...uiMap.energyPairCalculator,
    ...uiMap.energyPairPage,
    unitDescriptions: uiMap.unitDescriptions,
...
function buildPairContent(fromKey, toKey, uiMap, locale) {
  ...
  const page = uiMap.energyPairPage;
  ...
  const faqA2 =
    locale === "ko"
      ? "예. 수식 줄, 설명 섹션, 에너지 변환 표가 포함되어 있습니다."
      : "Yes. This page includes formula lines, explanation sections, and energy conversion tables.";
  const faqQ3 =
    locale === "ko"
      ? "다른 에너지 단위 쌍 페이지로 이동할 수 있나요?"
      : "Can I move to other energy pair pages?";
  const faqA3 =
    locale === "ko"
      ? "예. 아래 관련 에너지 단위 쌍 링크에서 이동할 수 있습니다."
      : "Yes. Cross-links to related energy unit pairs are provided below.";
  ...
  return {
    ...
    backToHub: page.backToEnergyHub,
...
for (const locale of ["en", "ko"]) {
  ...
  const uiMap = locale === "en" ? energyUiEn : energyUiKo;
  const hubSrc = locale === "en" ? energyHubContentEn : energyHubContentKo;
  ...
  console.log(`Patched ${locale}/toolContent.json: energy hub + ${pairCount} pair pages`);
}
