...
import { guideBaseMeta, guideFormat, guideText, type GuideBaseMeta } from "@/lib/number-system-guide-i18n";
...
type BaseMeta = GuideBaseMeta & { radix: number | null };

function getLocalizedBaseMeta(ui: unknown): Record<NumberSystemBase, BaseMeta> {
  const meta = guideBaseMeta(ui, asMap(asMap(ui).bases));
  return {
    "2": { ...meta["2"], radix: 2 },
    "8": { ...meta["8"], radix: 8 },
    "10": { ...meta["10"], radix: 10 },
    "16": { ...meta["16"], radix: 16 },
    char: { ...meta.char, radix: null },
  };
}
...
function buildPowerExpansionChalk(
  ...
  baseMeta: Record<NumberSystemBase, BaseMeta>,
  ui: unknown
): string | null {
  const decSuffix = guideText(ui, "decimalSuffix", "(decimal)");
  const c = cleanedNumericInput(raw, fromBase);
  const r = baseMeta[fromBase].radix;
  ...
  if (dot === -1) {
    ...
    const label = baseMeta[fromBase].label;
  ...
  lines.push(`${guideText(ui, "leftOfDot", "Left of . :")} ${intTerms.join(" + ")} = ${intSum}`);
  ...
  if (rawFrac.length > 0) {
    ...
    lines.push(`${guideText(ui, "rightOfDot", "Right of .:")} ${fracTerms.join(" + ")} = ${fv}`);
  }

  const total = intSum + fracSum;
  lines.push(`= ${total}  ${decSuffix}`);
  return lines.join("\n");
}
...
function buildDivisionChalk(decimal: number, toBase: "2" | "8" | "16", ui: unknown): string {
  ...
  if (intN === 0 && frac <= 1e-15) {
    ...
    lines.push("");
    lines.push(guideText(ui, "divReadRemZero", "Read remainders bottom → top: 0"));
    return lines.join("\n");
  }

  if (intN > 0) {
    lines.push(guideText(ui, "divWholePart", "Whole part — repeated division:"));
    let x = intN;
    const rems: number[] = [];
    let step = 0;
    ...
    lines.push("");
    lines.push(guideFormat(ui, "divReadRem", "Read remainders bottom → top → {digits}", { digits }));
  } else {
    lines.push(guideText(ui, "divWholeZero", "Whole part: 0"));
  }

  if (frac > 1e-12) {
    lines.push("");
    lines.push(guideFormat(ui, "divFracIntro", "Fractional part — multiply by {r}, integer of each product = next digit after .", { r }));
    let f = frac;
    ...
    lines.push(guideFormat(ui, "divFracDigits", "Digits after . (in order): .{digits}", { digits: fracDigits.join("") }));
  }

  return lines.join("\n");
}

function getStep2Title(ui: unknown, fromBase: NumberSystemBase, toBase: NumberSystemBase): string {
  const bridge = fromBase !== "10" && toBase !== "10";
  if (bridge) return guideText(ui, "step2BridgeTitle", "Step 2 — The Two-Step Method (via Decimal)");

  if (fromBase === "10" && toBase === "2") return guideText(ui, "step2DecToBin", "Step 2 — Repeated Division by 2");
  if (fromBase === "10" && toBase === "8") return guideText(ui, "step2DecToOct", "Step 2 — Repeated Division by 8");
  if (fromBase === "10" && toBase === "16") return guideText(ui, "step2DecToHex", "Step 2 — Repeated Division by 16");
  if (fromBase === "10" && toBase === "char") return guideText(ui, "step2DecToChar", "Step 2 — Decimal Code → Character");

  if (fromBase === "2" && toBase === "10") return guideText(ui, "step2BinToDec", "Step 2 — Binary Place Weights");
  if (fromBase === "8" && toBase === "10") return guideText(ui, "step2OctToDec", "Step 2 — Octal Place Weights");
  if (fromBase === "16" && toBase === "10") return guideText(ui, "step2HexToDec", "Step 2 — Hex Place Weights");
  if (fromBase === "char" && toBase === "10") return guideText(ui, "step2CharToDec", "Step 2 — Character → Decimal Code");

  if (fromBase === "char") return guideText(ui, "step2CharOther", "Step 2 — Character Code, Then Target Base");
  if (toBase === "char") return guideText(ui, "step2ToChar", "Step 2 — Decimal Value → Character");

  return guideText(ui, "step2Default", "Step 2 — Convert");
}
...
function HexDigitTable({ ui }: { ui: unknown }) {
  return (
    <div className="mt-3 overflow-x-auto rounded-lg border border-slate-600 bg-slate-950/50">
      <table className="w-full min-w-[280px] border-collapse text-left text-sm text-slate-300">
        <caption className="border-b border-slate-600 px-3 py-2 text-left text-xs font-medium text-slate-400">
          {guideText(ui, "hexTableCaption", "Hex digit → value (for place weights)")}
        </caption>
        <thead>
          <tr className="border-b border-slate-600 text-slate-400">
            <th className="px-3 py-2 font-semibold">{guideText(ui, "hexDigit", "Digit")}</th>
            <th className="px-3 py-2 font-semibold">{guideText(ui, "hexValue", "Value")}</th>
            <th className="px-3 py-2 font-semibold">{guideText(ui, "hexDigit", "Digit")}</th>
            <th className="px-3 py-2 font-semibold">{guideText(ui, "hexValue", "Value")}</th>
          </tr>
        </thead>
...
function DevNoteBadge({ notes, ui }: { notes: string[]; ui: unknown }) {
  ...
  return (
    <div className="mt-5 flex flex-col gap-2 rounded-lg border border-amber-900/50 bg-amber-950/20 px-3 py-2.5">
      <span className="inline-flex w-fit items-center rounded-full border border-amber-700/60 bg-amber-950/40 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider text-amber-200/90">
        {guideText(ui, "devNoteBadge", "Dev note")}
      </span>
...
function StepIdentify({
  ...
  const from = baseMeta[fromBase];
  const to = baseMeta[toBase];
  ...
  return (
    <div className="rounded-lg border border-slate-700/80 bg-slate-800/40 p-5">
      <h3 className="mb-3 text-base font-semibold text-slate-100">
        {guideText(ui, "step1Title", "Step 1 — Identify the symbols")}
      </h3>
      <p className="text-sm leading-relaxed text-slate-400">
        <strong className="font-medium text-slate-200">{guideText(ui, "inputLabel", "Input")}</strong> ({from.label}):{" "}
        {from.beginnerSymbols}
      </p>
      <p className="mt-3 text-sm leading-relaxed text-slate-400">
        <strong className="font-medium text-slate-200">{guideText(ui, "outputLabel", "Output")}</strong> ({to.label}):{" "}
        {to.beginnerSymbols}
      </p>
      {showHexTable && (
        <>
          <p className="mt-4 text-sm text-slate-400">{guideText(ui, "hexMapIntro", "Hex conversions use this letter-to-number map:")}</p>
          <HexDigitTable ui={ui} />
        </>
      )}
      <DevNoteBadge notes={devNotes} ui={ui} />
    </div>
  );
}
...
function StepProcess({
  ...
  const partAHeading = bridge ? guideText(ui, "step2PartA", "Part A — Into decimal") : null;
  const partBHeading = bridge ? guideText(ui, "step2PartB", "Part B — Out of decimal") : null;
      ...
      <>
        <p className="text-sm leading-relaxed text-slate-400">
          {guideFormat(
            ui,
            "placeWeightsBody",
            "Number positions from the right, starting at 0. At each position, multiply that digit by {radix} raised to the position index, then add every term. The total is your decimal number.",
            { radix }
          )}
        </p>
        <p className="mt-2 text-sm leading-relaxed text-slate-400">
          {guideFormat(
            ui,
            "fractionalBody",
            "Digits after the dot: use negative powers of {radix} ({radix}^-1, {radix}^-2, …). Each place is still (digit × weight); add the fractional side to the whole side.",
            { radix }
          )}
        </p>
        {extraHex}
  ...
  const title = getStep2Title(ui, fromBase, toBase);
        ...
        <div className="rounded-lg border border-slate-700/80 bg-slate-800/40 p-5">
          ...
          {fromBase === "16" && (
            <p className="mb-3 text-sm text-slate-400">
              {guideText(ui, "hexAfMapping", "A–F mapping: use 10–15 for letters when applying place weights.")}
            </p>
          )}
  ...
  return (
    <div className="rounded-lg border border-slate-700/80 bg-slate-800/40 p-5">
      ...
      {fromBase !== "10" ? (
        ...
        <p className="text-sm text-slate-400">{guideText(ui, "step2SkipPartB", "You already start in decimal — skip to Part B.")}</p>
      )}
      {partBHeading && <h4 className="mb-2 mt-5 text-sm font-semibold text-slate-200">{partBHeading}</h4>}
      {toBase !== "10" ? (
        renderDecimalToTarget()
      ) : (
        <p className="text-sm text-slate-400">{guideText(ui, "step2GoalDecimal", "Your goal is decimal — Part A already finished the job.")}</p>
      )}
    </div>
  );
}
...
function WorkedExampleBlock({
  ...
  try {
    ...
    return (
      <div className="rounded-lg border border-red-900/40 bg-red-950/20 p-4 text-sm text-red-300/90">
        {guideFormat(ui, "parseFail", "Example {n}: could not parse {raw} for this pair.", {
          n: exampleIndex,
          raw,
        })}
      </div>
  ...
  const fromName = baseMeta[fromBase].label;
  const toName = baseMeta[toBase].label;
  const showQuotes = fromBase !== "char";

  const showTowardDecimal = fromBase !== "10";
  const expansion = fromBase !== "char" ? buildPowerExpansionChalk(raw, fromBase, baseMeta, ui) : null;
  const towardCode =
    fromBase === "char"
      ? guideFormat(ui, "charToDecChalk", "Character '{raw}'\n→ code point (decimal) N₁₀ = {decimal}", {
          raw,
          decimal,
        })
      : expansion ?? `N₁₀ = ${decimal}`;
  ...
  const fromDecimalCode =
    toBase === "char"
      ? guideFormat(ui, "decToCharChalk", "Decimal {decimal}\n→ character display: {output}", { decimal, output })
      : toBase === "2" || toBase === "8" || toBase === "16"
        ? `${buildDivisionChalk(decimal, toBase, ui)}\n\n${guideFormat(ui, "toolResult", "→ tool: {output}", { output })}`
        : "";

  return (
    <div className={exampleIndex > 1 ? "mt-8 border-t border-slate-700 pt-8" : ""}>
      <h4 className="mb-2 text-sm font-semibold text-slate-200">
        {guideFormat(ui, "exampleN", "Example {n}", { n: exampleIndex })}
      </h4>
      <p className="mb-4 text-sm text-slate-500">
        {guideFormat(ui, "exampleArrow", "{raw} ({fromName}) → {toName}.", {
          raw: showQuotes ? `"${raw}"` : `‘${raw}’`,
          fromName,
          toName,
        })}
      </p>
      ...
      {showTowardDecimal && (
        <ChalkBlock label={guideText(ui, "towardDecimal", "Toward decimal")} code={towardCode} />
      )}

      {fromBase === "10" && showFromDecimalNonDec && fromDecimalCode && (
        <ChalkBlock
          label={guideText(ui, "fromDecimal", "From decimal")}
          code={`N₁₀ = ${decimal}\n\n${fromDecimalCode}`.trim()}
        />
      )}

      {fromBase !== "10" && showFromDecimalNonDec && fromDecimalCode && (
        <ChalkBlock label={guideText(ui, "fromDecimalToOutput", "From decimal to output")} code={fromDecimalCode} />
      )}
      ...
      <p className="mt-4 rounded-md border border-slate-600 bg-slate-950/50 px-3 py-2 text-center font-mono text-xs text-slate-400 sm:text-sm">
        {guideFormat(ui, "verify", "Verify: {raw} → {output}", {
          raw: showQuotes ? `"${raw}"` : raw,
          output,
        })}
      </p>
  ...
  return (
    <div className="rounded-lg border border-slate-700/80 bg-slate-800/40 p-5">
      <h3 className="mb-2 text-base font-semibold text-slate-100">
        {guideText(ui, "step3Title", "Step 3 — Worked examples")}
      </h3>
      <p className="mb-6 text-sm leading-relaxed text-slate-500">
        {guideFormat(ui, "step3Intro", "Two practice values in {fromName}, converted to {toName} using the same rules as Step 2.", {
          fromName,
          toName,
        })}
        {showFracExample ? <> {guideText(ui, "step3FracNote", "Example 3 uses a fractional part (digits after the radix point); the hub and pair calculators accept a single . on binary, octal, decimal, and hex inputs.")}</> : null}
      </p>

      <WorkedExampleBlock fromBase={fromBase} toBase={toBase} raw={raw1} exampleIndex={1} baseMeta={baseMeta} ui={ui} />
      <WorkedExampleBlock fromBase={fromBase} toBase={toBase} raw={raw2} exampleIndex={2} baseMeta={baseMeta} ui={ui} />
      {showFracExample && rawFrac ? (
        <WorkedExampleBlock fromBase={fromBase} toBase={toBase} raw={rawFrac} exampleIndex={3} baseMeta={baseMeta} ui={ui} />
      ) : null}
    </div>
  );
}

export default function ConversionGuide({ fromBase, toBase, className = "", title, ui }: ConversionGuideProps) {
  const baseMeta = getLocalizedBaseMeta(ui);
  ...
  return (
    ...
    >
      ...
      <div className="space-y-8">
        <StepIdentify fromBase={fromBase} toBase={toBase} baseMeta={baseMeta} ui={ui} />
        <StepProcess fromBase={fromBase} toBase={toBase} baseMeta={baseMeta} ui={ui} />
        <StepCalculate fromBase={fromBase} toBase={toBase} baseMeta={baseMeta} ui={ui} />
      </div>
    </section>
  );
}
