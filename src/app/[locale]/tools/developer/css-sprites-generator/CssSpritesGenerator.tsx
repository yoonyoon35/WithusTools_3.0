...
const META_PATH = "/tools/developer/css-sprites-generator";
...
export default function CssSpritesGenerator() {
  ...
  const handleFiles = useCallback(
    (files: FileList | null) => {
      ...
      if (!imageFiles.length) {
        showToast(asText(messages.selectImageFiles), "error");
        return;
      }
      Promise.all(imageFiles.map(loadImage))
        .then((loaded) => setImages((prev) => [...prev, ...loaded]))
        .catch(() => showToast(asText(messages.failedLoadImages), "error"));
    },
    [messages.failedLoadImages, messages.selectImageFiles, showToast]
  );
  ...
  const generateSprite = useCallback(() => {
    ...
    setCssCode(css);
    showToast(asText(messages.generated), "success");
  }, [bgColor, columns, images, messages.generated, padding, showToast]);
  ...
  }, [messages.downloaded, showToast, spriteDataUrl]);
  ...
  }, [cssCode, shared.copied, shared.copyFailed, showToast]);
  ...
  const reset = useCallback(() => {
    ...
    showToast(asText(shared.reset), "success");
  }, [shared.reset, showToast]);
    ...
    <div className="space-y-6" role="main" aria-label={asText(page?.h1)}>
      {images.length === 0 ? (
        <div className="rounded-xl border border-border bg-surface p-6">
          <h2 className="mb-4 text-lg font-semibold text-slate-100">{asText(labels.uploadFiles)}</h2>
          <div
        ...
        <>
          <div className="rounded-xl border border-border bg-surface p-6">
            <h3 className="mb-3 text-lg font-semibold text-slate-100">{asText(labels.images)}</h3>
            ...
            >
              {asText(actions.addMore)}
            </button>
          </div>

          <div className="rounded-xl border border-border bg-surface p-6">
            <h3 className="mb-3 text-lg font-semibold text-slate-100">{asText(labels.settings)}</h3>
            <div className="flex flex-wrap gap-4">
              <div>
                <label htmlFor="sprite-padding" className="mb-1 block text-sm text-slate-400">
                  {asText(labels.padding)}
                </label>
          ...
          {spriteDataUrl && (
            <div className="rounded-xl border border-border bg-surface p-6">
              <h3 className="mb-3 text-lg font-semibold text-slate-100">{asText(labels.spritePreview)}</h3>
              <img
                src={spriteDataUrl}
                alt="Sprite"
          ...
          {cssCode && (
            <div className="rounded-xl border border-border bg-surface p-6">
              <div className="mb-3 flex items-center justify-between">
                <h3 className="text-lg font-semibold text-slate-100">{asText(labels.cssCode)}</h3>
                <button
                  type="button"
                  onClick={copyCss}
                  aria-label="Copy CSS code"
