...
export function getConversionProcedureSteps(
  from: ColorFormatKey,
  to: ColorFormatKey,
  ui?: unknown
): string[] {
  const pageUi = asMap(ui);
  const localizedStep1 = asText(pageUi.procedureStep1);
  if (localizedStep1) {
    const f = colorFormatLabel(ui, from, "short");
    const t = colorFormatLabel(ui, to, "short");
    const steps = [
      formatUi(localizedStep1, { from: f, to: t }),
      formatUi(asText(pageUi.procedureStep2), { from: f, to: t }),
      formatUi(asText(pageUi.procedureStep3), { from: f, to: t }),
      formatUi(asText(pageUi.procedureStep4), { from: f, to: t }),
    ];
    const footnotes: string[] = [];
    if (from === "cmyk" || to === "cmyk") {
      const cmykFoot = asText(pageUi.procedureFootnoteCmyk);
      if (cmykFoot) footnotes.push(cmykFoot);
    }
    if (from === "hsl" || to === "hsl" || from === "hsv" || to === "hsv") {
      const hslFoot = asText(pageUi.procedureFootnoteHsl);
      if (hslFoot) footnotes.push(hslFoot);
    }
    return [...steps, ...footnotes];
  }
