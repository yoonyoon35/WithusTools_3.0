import { convertColorString, type ColorFormatKey } from "@/utils/colorFormatConversions";
...
export default function ColorConversionExamples({
  ...
  return (
    <section className="mt-10 rounded-xl border border-border bg-surface p-6 sm:p-8">
      <h2 className="mb-4 text-lg font-semibold text-slate-200">{asText(pageUi.examplesTitle)}</h2>
      <p className="mb-4 text-sm text-slate-500">
        {formatUi(asText(pageUi.examplesIntro), { from: fromShort, to: toShort })}
      </p>
          ...
          <thead>
            <tr className="border-b border-slate-600 text-slate-400">
              <th className="py-2 pr-4 font-medium">
                {fromShort} {asText(pageUi.tableInputSuffix)}
              </th>
              <th className="py-2 font-medium">
                {toShort} {asText(pageUi.tableOutputSuffix)}
              </th>
            </tr>
          </thead>
          <tbody>
