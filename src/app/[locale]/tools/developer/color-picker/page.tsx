...
export default async function ColorPickerPage({
  ...
  const colorPickerFaqLinks = getFaqEntriesByCategory("color-picker", locale);
  ...
  const guideIntroNode =
    content.guideIntroBefore && content.guideIntroLink1 ? (
      <p className="mb-6 text-sm leading-relaxed text-slate-400">
        ...
        <Link href="/tools/developer/css-sprites-generator" className="underline hover:text-slate-200">
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      ...
      <ColorPicker
        dedicatedConvertersFaq={colorPickerFaqLinks.map((e) => ({
          slug: e.slug,
          question: e.question,
          category: e.category,
        }))}
