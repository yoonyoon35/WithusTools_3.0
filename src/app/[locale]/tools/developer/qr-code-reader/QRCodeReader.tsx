...
const META_PATH = "/tools/developer/qr-code-reader";

export default function QRCodeReader() {
  const page = useToolPageContent(META_PATH);
  const ui = page?.ui;
  const toolUi = asMap(ui);
  const modes = asMap(toolUi.modes);
  ...
  const startCamera = useCallback(async () => {
    try {
      ...
      try {
        ...
        showToast(asText(messages.cameraDenied), "error");
      }
    }
  }, [messages.cameraDenied, showToast]);
  ...
  useEffect(() => {
    ...
    return () => stopCamera();
  }, [mode, scanFrame, startCamera, stopCamera]);
    ...
    (file: File) => {
      ...
      img.onload = () => {
        try {
          ...
          if (!code) showToast(asText(messages.noQr), "error");
        } finally {
      ...
      img.onerror = () => {
        URL.revokeObjectURL(url);
        showToast(asText(messages.imageLoadFailed), "error");
      };
      img.src = url;
    },
    [messages.imageLoadFailed, messages.noQr, showToast]
  );
  ...
  const addFile = useCallback(
    (fileList: FileList | null) => {
      ...
      if (!file.type.startsWith("image/")) {
        showToast(asText(messages.imageOnly), "error");
        return;
      }
      processFile(file);
      if (fileInputRef.current) fileInputRef.current.value = "";
    },
    [messages.imageOnly, processFile, showToast]
  );
  ...
  const copyResult = useCallback(() => {
    ...
    navigator.clipboard
      .writeText(result)
      .then(() => showToast(asText(toolUi.copied)))
      .catch(() => showToast(asText(toolUi.copyFailed), "error"));
  }, [result, showToast, toolUi.copied, toolUi.copyFailed]);
  ...
  const openResult = useCallback(() => {
    ...
    try {
      ...
      showToast(asText(messages.invalidUrl), "error");
    }
  }, [messages.invalidUrl, result, showToast]);
    ...
    <div className="space-y-6">
      <div className="flex gap-2">
        <button
          ...
          aria-label={asText(aria.cameraMode)}
        ...
        <button
          ...
          aria-label={asText(aria.uploadMode)}
          className={`rounded-lg px-4 py-2 text-sm font-medium ${
            mode === "upload"
              ? "border border-blue-500 bg-blue-500/20 text-blue-300"
              : "border border-border text-slate-400 hover:border-slate-600 hover:text-slate-200"
        ...
        <div className="rounded-xl border border-border bg-surface p-6">
          <div className="relative overflow-hidden rounded-lg bg-black">
            ...
            <div className="absolute inset-0 flex items-center justify-center border-4 border-dashed border-white/30">
              <p className="text-sm text-white/80">{asText(labels.frameHint)}</p>
            </div>
          </div>
        </div>
      )}
      ...
      {mode === "upload" && (
        <div className="rounded-xl border border-border bg-surface p-6">
          <h2 className="mb-4 text-lg font-semibold text-slate-100">{asText(labels.uploadImage)}</h2>
          <div
            role="button"
            tabIndex={0}
            ...
            aria-label={asText(aria.dropZone)}
          ...
          >
            ...
            <p className="mb-2 text-sm text-slate-400">
              {asText(labels.dropZone)}
            </p>
            <p className="text-xs text-slate-500">{asText(labels.dropZoneHint)}</p>
            <input
              ref={fileInputRef}
              type="file"
              accept="image/*"
      ...
      <div className="rounded-xl border border-border bg-surface p-6">
        <h3 className="mb-3 text-lg font-semibold text-slate-100">{asText(labels.scannedResult)}</h3>
        {result ? (
          <div className="space-y-3">
            ...
            <div className="flex gap-2">
              <button
                type="button"
                onClick={copyResult}
                aria-label={asText(aria.copyResult)}
              ...
              <button
                type="button"
                onClick={openResult}
                aria-label={asText(aria.openResult)}
                className={`rounded-lg border border-border px-4 py-2 text-sm text-slate-300 hover:border-slate-600 hover:bg-slate-700 ${focusRing}`}
              >
                {asText(actions.openIfUrl)}
              </button>
            </div>
          </div>
        ) : (
          <p className="text-slate-500">{asText(labels.emptyResult)}</p>
        )}
      </div>
