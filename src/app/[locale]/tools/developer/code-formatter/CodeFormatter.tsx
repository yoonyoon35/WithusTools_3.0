...
const META_PATH = "/tools/developer/code-formatter";
...
export default function CodeFormatter() {
  ...
  const languages = asMap(toolUi.languages);
  ...
  const formatCode = useCallback(async () => {
    ...
    if (!trimmed) {
      showToast(asText(messages.needCode), "error");
      return;
    }
    try {
      const lang = language;
      ...
      setCode(formatted);
      showToast(asText(messages.formatted), "success");
    } catch (err) {
  ...
  }, [code, messages.copied, shared.copyFailed, showToast]);

  const clearCode = useCallback(() => {
    if (code.trim() && window.confirm(asText(messages.clearConfirm))) {
      setCode("");
      showToast(asText(shared.cleared), "success");
    }
  }, [code, messages.clearConfirm, shared.cleared, showToast]);
      ...
      <div className="rounded-xl border border-border bg-surface p-6">
        <div className="mb-3 flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            >
              <option value="javascript">{asText(languages.javascript)}</option>
              <option value="html">{asText(languages.html)}</option>
              <option value="css">{asText(languages.css)}</option>
              <option value="json">{asText(languages.json)}</option>
            </select>
          </div>
        ...
        <textarea
          ...
          placeholder={asText(placeholders.input)}
