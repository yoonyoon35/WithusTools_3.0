...
function metaPathFor(format?: ImageToPdfFormat): string {
  return format ? `/tools/pdf-converter/${format}` : `/tools/pdf/image-to-pdf`;
}
...
export default function ImageToPdfConverter({
  ...
  const page = useToolPageContent(metaPathFor(format));
  const ui = page?.ui;
  const toolUi = asMap(ui);
  const messagesUi = asMap(toolUi.messages);
  const acceptedFormatsLabel =
    asText(page?.displayName) || labelForFormat(format);
  const t = (key: string, fallback: string) => asText(toolUi[key]) || fallback;
  const m = (key: string, fallback: string) => asText(messagesUi[key]) || fallback;
  ...
  const addFiles = useCallback(
    (fileList: FileList | null) => {
      ...
      if (arr.length === 0) {
        showMessage(
          format === undefined
            ? m("addSupportedImage", "Please add at least one supported image file.")
            : formatUi(m("selectFormatImages", "Please select {format} images."), {
                format: acceptedFormatsLabel,
              }),
          "error"
        );
        return;
    ...
    [files, showMessage, format, acceptedFormatsLabel, messagesUi]
  );
  ...
  const convert = useCallback(async () => {
    if (files.length === 0) {
      showMessage(m("addImagesFirst", "Please add images first."), "error");
      return;
    }
    setLoading(true);
    setPdfUrl(null);
    ...
    try {
      ...
      showMessage(m("pdfCreated", "PDF created successfully."));
    } catch (err) {
      showMessage(
        err instanceof Error ? err.message : m("convertFailed", "Failed to convert to PDF."),
        "error"
      );
    } finally {
      setLoading(false);
  ...
  }, [files, pageSize, orientation, showMessage, format, messagesUi]);
      ...
      <div className="rounded-xl border border-border bg-surface p-6">
        ...
        >
          ...
          <p className="mb-2 text-sm text-slate-400">
            {format === undefined
              ? t("dropImages", "Drop images here or click to upload")
              : formatUi(
                  t("dropFormat", "Drop {format} images here or click to upload"),
                  { format: acceptedFormatsLabel }
                )}
          </p>
          <p className="text-xs text-slate-500">
            {format === undefined
              ...
              : t("supportsMultiple", "Supports multiple files")}
          </p>
          <input
            ref={fileInputRef}
            type="file"
          ...
          <div className="mt-4 space-y-4">
            <div className="mb-4">
              <h3 className="mb-3 text-sm font-medium text-slate-400">
                {formatUi(t("uploadedFiles", "Uploaded files ({n})"), {
                  n: files.length,
                })}
              </h3>
              {previewError && (
                <div className="mb-3 space-y-1 rounded-lg border border-amber-500/30 bg-amber-500/10 p-3 text-xs text-amber-300">
                  <p role="alert">
                    {formatUi(t("previewWarning", "Preview warning: {text}"), {
                      text: previewError,
                    })}
                  </p>
                  {previewFailures.length > 0 && (
                    <p className="text-amber-200/80">
                      {formatUi(
                        t("failedPreviewList", "Failed ({n}): {names}"),
                        {
                          n: previewFailures.length,
                          names:
                            previewFailures
                              .slice(0, 3)
                              .map((f) => f.name)
                              .join(", ") + (previewFailures.length > 3 ? ", …" : ""),
                        }
                      )}
                    </p>
                  )}
              ...
              {!previewsReady && (
                <p className="mb-3 flex items-center gap-2 text-xs text-slate-500">
                  <span
                    ...
                    aria-hidden
                  />
                  {t("generatingPreviews", "Generating previews…")}
                </p>
              )}
              <div
                className={
                      ...
                      {previewsReady && convertedPreviews[i] ? (
                        ...
                        <div className="flex h-full min-h-[120px] flex-col items-center justify-center gap-2 bg-slate-900 p-3 text-center text-xs text-slate-500">
                          <p className="font-medium text-slate-400">
                            {t("couldNotDecode", "Could not decode preview")}
                          </p>
                          ...
                          <p className="text-[11px] text-slate-500">
                            {t(
                              "stillConvertHint",
                              "You can still convert to PDF. If conversion fails, remove this file."
                            )}
                          </p>
                        </div>
                      ) : (
            ...
            <div className="mb-4 rounded-lg border border-border bg-slate-950/30 p-4">
                ...
                <div className="flex gap-4">
                  ...
                  <label className="flex cursor-pointer items-center gap-2">
                    <input
                      ...
                      className="rounded-full border-slate-600 bg-slate-800 text-blue-600 focus:ring-blue-500"
                    />
                    <span className="text-slate-300">{t("a4", "A4")}</span>
                  </label>
                </div>
                <p className="mt-1 text-xs text-slate-500">
                  {pageSize === "fit"
                    ? t("fitHint", "Each page matches its image size (no margins)")
                    : t(
                        "a4Hint",
              ...
              {pageSize === "a4" && (
                <div>
                  <label className="mb-2 block text-sm text-slate-400">
                    {t("pageOrientation", "Page orientation")}
                  </label>
                  <div className="flex gap-4">
                    <label className="flex cursor-pointer items-center gap-2">
                      <input
            ...
            <div className="flex flex-wrap items-center gap-4">
              ...
              >
                {loading
                  ...
                  : t("convertButton", "Convert to PDF")}
              </button>
              <button
                onClick={clearAll}
            ...
            {loading && progress && (
              <div className="mt-4 space-y-2">
                <div className="flex items-center justify-between text-sm text-slate-400">
                  <span>
                    {formatUi(
                      t("convertingProgress", "Converting... {current} of {total}"),
            ...
            {!loading && (
              <p className="mt-4 text-sm text-slate-500">
                {formatUi(t("filesSelected", "{n} file(s) selected"), {
                  n: files.length,
                })}
              </p>
            )}
      ...
      {pdfUrl && (
        <div className="rounded-xl border border-border bg-surface p-6">
          <p className="mb-3 font-semibold text-slate-100">
            {t("pdfCreatedTitle", "PDF created successfully")}
          </p>
