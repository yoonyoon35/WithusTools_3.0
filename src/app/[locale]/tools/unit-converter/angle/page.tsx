...
import { getCanonicalAngleSlug, ANGLE_HUB_KEYS } from "@/utils/conversions";
import { angleUnitLabel, angleUnitSlug } from "./anglePairUi";

const META_PATH = "/tools/unit-converter/angle";
...
export default async function AngleConverterPage({
  params,
}: {
  params: { locale: string };
}) {
  ...
  const angleFaqLinks = getFaqEntriesByCategory("angle", locale);
  const converterTitle = asText(toolUi.converterTitle) || "Convert Angle";
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      ...
      <UnitConverter category="angle" title={converterTitle} ui={content.ui} />

      <section className="mt-12 rounded-xl border border-border bg-surface p-6 sm:p-8">
        <h2 className="mb-4 text-lg font-semibold text-slate-200">{asText(toolUi.pairGridTitle)}</h2>
        <p className="mb-6 text-sm text-slate-500">
          {formatToolUiString(asText(toolUi.pairGridDesc), { count: ANGLE_PAIR_LINKS.length })}
        </p>
        <ul className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {ANGLE_PAIR_LINKS.map(({ from, to }) => {
            const href = `/tools/unit-converter/angle/${getCanonicalAngleSlug(from, to)}`;
            const fromName = angleUnitLabel(content.ui, from, "nameSg");
            const toName = angleUnitLabel(content.ui, to, "nameSg");
            const fromSlug = angleUnitSlug(from);
            const toSlug = angleUnitSlug(to);
            return (
              <li key={`${from}-${to}`}>
                <Link
                  href={href}
        ...
        <div className="mt-10 border-t border-slate-700 pt-8">
          ...
          <p className="mb-4 text-sm text-slate-500">
            {formatToolUiString(asText(toolUi.faqSectionDesc), { count: angleFaqLinks.length })}
          </p>
          <ul className="grid gap-2 sm:grid-cols-2">
            {angleFaqLinks.map((faq) => (
              <li key={faq.slug}>
                <Link
