...
import {
  ...
  formatEnergyResult,
  formatWithThousands,
  getEnergyFormulaLine,
} from "@/utils/conversions";
import { energyUnitLabel } from "./energyPairUi";

interface EnergyPairCalculatorProps {
  fromKey: string;
  toKey: string;
  metaPath?: string;
}
...
function normalizeMetaPath(pathname: string): string {
  ...
  return noLocale.startsWith("/tools/") ? noLocale : "/tools/unit-converter/energy/kilowatt-hour-to-joule";
}

export default function EnergyPairCalculator({ fromKey, toKey, metaPath }: EnergyPairCalculatorProps) {
  ...
  const fromLabel = energyUnitLabel(ui, fromKey, "nameSg");
  const toLabel = energyUnitLabel(ui, toKey, "nameSg");
  ...
  const convert = useCallback(() => {
    ...
    const result = convertEnergy(val, fromKey, toKey);
    setToValue(formatEnergyResult(result));
    setFormulaText(getEnergyFormulaLine(val, fromKey, toKey));
