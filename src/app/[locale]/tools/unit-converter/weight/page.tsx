...
import { getCanonicalWeightSlug, WEIGHT_HUB_KEYS } from "@/utils/conversions";
import { weightUnitLabel, weightUnitSlug } from "./weightPairUi";

const META_PATH = "/tools/unit-converter/weight";
...
export default async function WeightConverterPage({
  ...
  const weightFaqLinks = getFaqEntriesByCategory("weight", locale);
  const converterTitle = asText(toolUi.converterTitle) || "Convert Weight";
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      ...
      <UnitConverter category="weight" title={converterTitle} ui={content.ui} />

      <section className="mt-12 rounded-xl border border-border bg-surface p-6 sm:p-8">
        <h2 className="mb-4 text-lg font-semibold text-slate-200">{asText(toolUi.pairGridTitle)}</h2>
        <p className="mb-6 text-sm text-slate-500">
          {formatToolUiString(asText(toolUi.pairGridDesc), { count: WEIGHT_PAIR_LINKS.length })}
        </p>
        <ul className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {WEIGHT_PAIR_LINKS.map(({ from, to }) => {
            const href = `/tools/unit-converter/weight/${getCanonicalWeightSlug(from, to)}`;
            const fromName = weightUnitLabel(content.ui, from, "nameSg");
            const toName = weightUnitLabel(content.ui, to, "nameSg");
            const fromSlug = weightUnitSlug(from);
            const toSlug = weightUnitSlug(to);
            return (
              <li key={`${from}-${to}`}>
                <Link
                  href={href}
        ...
        <div className="mt-10 border-t border-slate-700 pt-8">
          ...
          <p className="mb-4 text-sm text-slate-500">
            {formatToolUiString(asText(toolUi.faqSectionDesc), { count: weightFaqLinks.length })}
          </p>
          <ul className="grid gap-2 sm:grid-cols-2">
            {weightFaqLinks.map((faq) => (
              <li key={faq.slug}>
                <Link
