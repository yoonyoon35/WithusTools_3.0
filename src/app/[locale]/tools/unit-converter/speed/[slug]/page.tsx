...
export async function generateMetadata({
  ...
  const title =
    content?.h1 ??
    `${speedUnitLabel(content?.ui, from, "nameSg")} to ${speedUnitLabel(content?.ui, to, "nameSg")} Converter`;
  const description =
    content?.intro ??
  ...
  return createMetadata({
    ...
    description,
    path: metaPath,
    keywords: [from, to, "speed converter", "velocity conversion", "withustools"],
    locale: locale as Locale,
  });
}
...
export default async function SpeedPairPage({
  params,
}: {
  params: { locale: string; slug: string };
}) {
  ...
  const fromSg = speedUnitLabel(content.ui, fromKey, "nameSg");
  const toSg = speedUnitLabel(content.ui, toKey, "nameSg");
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      ...
      >
        <SpeedPairCalculator fromKey={fromKey} toKey={toKey} metaPath={metaPath} />
      </Suspense>
      ...
      <div className="mt-10">
        <HowToConvertSpeed fromKey={fromKey} toKey={toKey} ui={content.ui} />
      </div>
      ...
      <section className="mt-10 rounded-xl border border-border bg-surface p-6 sm:p-8">
        ...
        <SpeedConversionTablesPair fromKey={fromKey} toKey={toKey} ui={content.ui} />
      </section>
