...
import {
  ...
  formatVolumeResult,
  formatWithThousands,
  getVolumeFormulaLine,
} from "@/utils/conversions";
import { volumeUnitLabel } from "./volumePairUi";

interface VolumePairCalculatorProps {
  fromKey: string;
  toKey: string;
  metaPath?: string;
}
...
function normalizeMetaPath(pathname: string): string {
  ...
  return noLocale.startsWith("/tools/") ? noLocale : "/tools/unit-converter/volume/liter-to-us-gallon";
}

export default function VolumePairCalculator({ fromKey, toKey, metaPath }: VolumePairCalculatorProps) {
  ...
  const fromLabel = volumeUnitLabel(ui, fromKey, "nameSg");
  const toLabel = volumeUnitLabel(ui, toKey, "nameSg");
  ...
  const convert = useCallback(() => {
    ...
    const result = convertVolume(val, fromKey, toKey);
    setToValue(formatVolumeResult(result));
    setFormulaText(getVolumeFormulaLine(val, fromKey, toKey));
