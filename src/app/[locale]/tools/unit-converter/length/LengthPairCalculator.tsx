...
import {
  ...
  formatLengthResult,
  formatWithThousands,
  getLengthFormulaLine,
} from "@/utils/conversions";
import { lengthUnitLabel } from "./lengthPairUi";

interface LengthPairCalculatorProps {
  fromKey: string;
  toKey: string;
  metaPath?: string;
}
...
function normalizeMetaPath(pathname: string): string {
  ...
  return noLocale.startsWith("/tools/") ? noLocale : "/tools/unit-converter/length/m-to-feet";
}

export default function LengthPairCalculator({ fromKey, toKey, metaPath }: LengthPairCalculatorProps) {
  ...
  const fromLabel = lengthUnitLabel(ui, fromKey, "nameSg");
  const toLabel = lengthUnitLabel(ui, toKey, "nameSg");
  ...
  const convert = useCallback(() => {
    ...
    const result = convertLength(val, fromKey, toKey);
    setToValue(formatLengthResult(result));
    setFormulaText(getLengthFormulaLine(val, fromKey, toKey));
