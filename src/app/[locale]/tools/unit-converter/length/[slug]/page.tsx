...
export async function generateMetadata({
  ...
  const metaPath = `/tools/unit-converter/length/${params.slug}`;
  const content = getToolContentEntry(toolContent, metaPath);
  const { from, to } = pair;

  const title =
    content?.h1 ??
    `${lengthUnitLabel(content?.ui, from, "nameSg")} to ${lengthUnitLabel(content?.ui, to, "nameSg")} Converter`;
  const description =
    content?.intro ??
  ...
  return createMetadata({
    ...
    description,
    path: metaPath,
    keywords: [from, to, "length converter", "unit conversion", "withustools"],
    locale: locale as Locale,
  });
}
...
export default async function LengthPairPage({
  params,
}: {
  params: { locale: string; slug: string };
}) {
  ...
  const fromSg = lengthUnitLabel(content.ui, fromKey, "nameSg");
  const toSg = lengthUnitLabel(content.ui, toKey, "nameSg");
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      ...
      <LengthPairCalculator fromKey={fromKey} toKey={toKey} metaPath={metaPath} />
      ...
      <div className="mt-10">
        <HowToConvertLength fromKey={fromKey} toKey={toKey} ui={content.ui} />
      </div>
      ...
      <section className="mt-10 rounded-xl border border-border bg-surface p-6 sm:p-8">
        ...
        <LengthConversionTablesPair fromKey={fromKey} toKey={toKey} ui={content.ui} />
      </section>
