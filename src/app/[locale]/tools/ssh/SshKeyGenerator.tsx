...
function mapErrorMessage(message: string, ui: Record<string, string>): string {
  const known: Record<string, string | undefined> = {
    "Key generation failed": ui.errorKeyGenFailed,
    "DSA key generation is not yet supported. Please use RSA, Ed25519, or ECDSA.":
      ui.errorDsaUnsupported,
    "Invalid passphrase or key": ui.errorInvalidPassphrase,
    "Could not extract EC public key": ui.errorExtractEcKey,
    "Could not parse EC key": ui.errorCouldNotParseEcKey,
...
export default function SshKeyGenerator({
  ...
  const algoContent = useToolPageContent(`/tools/ssh/${algorithm}`);
  ...
  const generateKeys = useCallback(async () => {
    ...
    try {
      if (algorithm === "rsa") {
        ...
        setError(ui.errorDsaUnsupported);
      }
    } catch (err) {
      const raw = err instanceof Error ? err.message : ui.errorKeyGenFailed;
      setError(mapErrorMessage(raw, ui));
    } finally {
      setLoading(false);
    }
  }, [algorithm, rsaKeySize, ecdsaCurve, passphrase, ui]);

  const verifyKeyPair = useCallback(async () => {
    if (!ui || !privateKey || !publicKey) return;
    setError("");
    setVerified(null);
  ...
  }, [privateKey, publicKey, algorithm, ecdsaCurve, passphrase, ui]);

  useEffect(() => {
    if (showPageHeader && algoContent?.h1) {
      document.title = `${algoContent.h1} | WithusTools`;
    }
  }, [showPageHeader, algoContent?.h1]);
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="text-center">
              <h1 className="text-3xl font-bold text-slate-100">
                {algoContent?.h1 ?? `${ALGORITHM_DISPLAY_NAMES[algorithm]} SSH Key Generator`}
              </h1>
              <p className="mt-1 text-sm text-slate-500">
                {algoContent?.subtitle ?? ""}
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="rounded-xl border border-border bg-surface p-6">
        <h2 className="mb-4 text-lg font-semibold text-slate-100">{ui.algorithm}</h2>
        ...
        {(algorithm === "rsa" || algorithm === "ecdsa") && (
          <div className="mt-6">
            <h2 className="mb-4 text-lg font-semibold text-slate-100">{ui.keySize}</h2>
            <div className="flex flex-wrap gap-2">
              {algorithm === "rsa" &&
      ...
      <div className="rounded-xl border border-border bg-surface p-6">
        <h2 className="mb-2 text-lg font-semibold text-slate-100">{ui.passphraseTitle}</h2>
        <p className="mb-3 text-sm text-slate-400">{ui.passphraseHelp}</p>
        <input
          type="password"
          value={passphrase}
        ...
        {algorithm === "ed25519" && passphrase && (
          <p className="mt-2 text-sm text-amber-400">{ui.ed25519PassphraseWarning}</p>
        )}
      </div>
      ...
      <div className="rounded-xl border border-border bg-surface p-6">
        ...
        >
          {loading ? ui.generating : ui.generateKeyPair}
        </button>
      </div>
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-xl border border-border bg-surface p-6">
            <div className="mb-2 flex items-center justify-between">
              <h3 className="font-semibold text-slate-100">{ui.privateKey}</h3>
              <button onClick={() => copyToClipboard(privateKey, "private")} className="rounded-lg border border-border px-3 py-1.5 text-sm text-slate-300 transition-colors hover:bg-slate-700">
                {copied === "private" ? ui.copied : ui.copy}
              </button>
            </div>
          ...
          <div className="rounded-xl border border-border bg-surface p-6">
            <div className="mb-2 flex items-center justify-between">
              <h3 className="font-semibold text-slate-100">{ui.publicKey}</h3>
              <button onClick={() => copyToClipboard(publicKey, "public")} className="rounded-lg border border-border px-3 py-1.5 text-sm text-slate-300 transition-colors hover:bg-slate-700">
                {copied === "public" ? ui.copied : ui.copy}
              </button>
            </div>
          ...
          {fingerprint && (
            <div className="rounded-xl border border-border bg-surface p-6">
              <div className="mb-2 flex items-center justify-between">
                <h3 className="font-semibold text-slate-100">{ui.fingerprint}</h3>
                <button onClick={() => copyToClipboard(fingerprint, "fingerprint")} className="rounded-lg border border-border px-3 py-1.5 text-sm text-slate-300 transition-colors hover:bg-slate-700">
                  {copied === "fingerprint" ? ui.copied : ui.copy}
                </button>
              </div>
      ...
      {showAlgorithmGuide && algoContent?.sections?.length ? (
        <ToolPageGuide
          title={algoContent.guideTitle}
          intro={algoContent.guideIntro}
          sections={algoContent.sections}
        />
      ) : null}
