...
const META_PATH = "/tools/image/paint-board";
...
export default function PaintBoard() {
  ...
  const toolsUi = asMap(toolUi.tools);
  const colorsUi = asMap(toolUi.colors);
  const historyUi = asMap(toolUi.history);
  ...
  const toolLabels: Record<Tool, string> = {
    brush: asText(toolsUi.brush),
    eraser: asText(toolsUi.eraser),
    rectangle: asText(toolsUi.rectangle),
    circle: asText(toolsUi.circle),
    line: asText(toolsUi.line),
    image: asText(toolsUi.insertImage),
  };
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-center gap-6 rounded-xl border border-border bg-surface p-4">
        <div className="flex items-center gap-2">
          {(
            [
              ...
              ["eraser", "M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"],
              ["rectangle", "M4 5a1 1 0 011-1h14a1 1 0 011 1v14a1 1 0 01-1 1H5a1 1 0 01-1-1V5z"],
        ...
        <div className="flex items-center gap-2 border-l border-slate-600 pl-4">
          <label className="flex h-10 w-10 items-center justify-center text-slate-400" title={asText(colorsUi.foreground)}>
          ...
          />
          <label className="flex h-10 w-10 items-center justify-center text-slate-400" title={asText(colorsUi.background)}>
            <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 20 20">
              <path
                fillRule="evenodd"
          ...
          <span className="min-w-[3rem] text-center text-sm text-slate-400">
            {formatUi(asText(toolUi.brushSize), { n: brushSize })}
          </span>
        </div>
        <div className="flex items-center gap-2 border-l border-slate-600 pl-4">
          <button
      ...
      {pendingImage && (
        <p className="text-center text-sm text-blue-400">{asText(toolUi.pendingImage)}</p>
      )}
    </div>
  );
}
