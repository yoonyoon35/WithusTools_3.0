...
const META_PATH = "/tools/image/image-format-converter";

type FormatMime = "image/png" | "image/jpeg" | "image/webp" | "image/gif" | "image/bmp" | "image/tiff" | "image/avif";

const FORMAT_TECH: Record<
  FormatMime,
  { hasQuality: boolean; supportsTransparency: boolean; uiKey: string }
> = {
  "image/png": { hasQuality: false, supportsTransparency: true, uiKey: "png" },
  "image/jpeg": { hasQuality: true, supportsTransparency: false, uiKey: "jpeg" },
  "image/webp": { hasQuality: true, supportsTransparency: true, uiKey: "webp" },
  "image/gif": { hasQuality: false, supportsTransparency: true, uiKey: "gif" },
  "image/bmp": { hasQuality: false, supportsTransparency: false, uiKey: "bmp" },
  "image/tiff": { hasQuality: false, supportsTransparency: true, uiKey: "tiff" },
  "image/avif": { hasQuality: true, supportsTransparency: true, uiKey: "avif" },
};

const FORMAT_MIMES = Object.keys(FORMAT_TECH) as FormatMime[];
...
export default function ImageFormatConverter() {
  ...
  const formatOptionsUi = asMap(toolUi.formatOptions);
  const formatDescriptionsUi = asMap(toolUi.formatDescriptions);
  ...
  const loadImage = useCallback(
    async (file: File) => {
      ...
      try {
        ...
        img.onerror = () => {
          showMessage(asText(messagesUi.loadFailed), "error");
          setIsLoading(false);
        };
        img.src = dataUrl;
      } catch (err) {
        showMessage(
          err instanceof Error ? err.message : asText(messagesUi.loadError),
          "error"
        );
        setIsLoading(false);
      }
    },
    [loadHEICImage, showMessage, messagesUi]
  );
  ...
  const convertImage = useCallback(() => {
    ...
    const ctx = canvas.getContext("2d");
    if (!ctx) {
      showMessage(asText(messagesUi.canvasError), "error");
      return;
    }
    ...
    const settings = FORMAT_TECH[format];
    ...
    try {
      ...
      showMessage(asText(messagesUi.converted));
    } catch {
      showMessage(asText(messagesUi.formatUnsupported), "error");
    }
  }, [originalImage, originalFile, format, quality, bgColor, showMessage, messagesUi]);
  ...
  const settings = FORMAT_TECH[format];
  const formatDescription = asText(formatDescriptionsUi[settings.uiKey]);
    ...
    <div className="space-y-6">
      ...
      {!originalDataUrl ? (
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_300px]">
          <div className="rounded-xl border border-border bg-surface p-6">
            <p className="mb-3 text-sm font-medium text-slate-300">{asText(toolUi.preview)}</p>
            <div className="aspect-video overflow-hidden rounded-lg bg-slate-800">
              {displayUrl && (
                <img
                  src={displayUrl}
                  alt={asText(toolUi.preview)}
            ...
            {originalFile && originalImage && (
              <div className="mt-4 text-sm text-slate-500">
                {formatUi(asText(fileInfoUi.originalFormat), { format: getFormatLabel(originalFile) })}
                <br />
                {formatUi(asText(fileInfoUi.size), { size: formatSize(originalFile.size) })}
                <br />
                {formatUi(asText(fileInfoUi.dimensions), {
                ...
                {convertedDataUrl && estimatedSize !== null && (
                  <>
                    <br />
                    {formatUi(asText(fileInfoUi.convertedSize), { size: formatSize(estimatedSize) })}
                  </>
                )}
              </div>
            )}
          ...
          <div className="rounded-xl border border-border bg-surface p-6">
            <div className="space-y-4">
              <div>
                ...
                >
                  {FORMAT_MIMES.map((mime) => (
                    <option key={mime} value={mime}>
                      {asText(formatOptionsUi[FORMAT_TECH[mime].uiKey])}
                    </option>
                  ))}
                </select>
              </div>
              ...
              <p className="text-xs italic text-slate-500">{formatDescription}</p>
