...
const META_PATH = "/tools/image/image-editor";
...
export default function ImageEditor() {
  ...
  const textModalUi = asMap(toolUi.textModal);
  const sectionsUi = asMap(toolUi.sections);
  const uploadUi = asMap(toolUi.upload);
  const filtersUi = asMap(toolUi.filters);
  const effectsUi = asMap(toolUi.effects);
  const transformUi = asMap(toolUi.transform);
  const drawUi = asMap(toolUi.draw);
  const utilitiesUi = asMap(toolUi.utilities);
  const positionsUi = asMap(utilitiesUi.positions);
  const actionsUi = asMap(toolUi.actions);
  ...
  const watermarkPositionOptions: { val: WatermarkPosition; labelKey: string }[] = [
    { val: "top-left", labelKey: "topLeft" },
    { val: "top-right", labelKey: "topRight" },
    { val: "center", labelKey: "center" },
    { val: "bottom-left", labelKey: "bottomLeft" },
    { val: "bottom-right", labelKey: "bottomRight" },
    { val: "tiled", labelKey: "tiled" },
  ];

  const instantEffectKeys: Exclude<InstantEffect, "none">[] = [
    "grayscale",
    "sepia",
    "invert",
    "noir",
    ...
    <div className="space-y-6">
      {showTextInput && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
          <div className="w-full max-w-md rounded-xl border border-border bg-surface p-4 shadow-xl">
            <label className="mb-2 block text-sm font-medium text-slate-200">{asText(textModalUi.label)}</label>
            <input
              type="text"
              value={textInput}
              ...
              placeholder={asText(textModalUi.placeholder)}
      ...
      {!showEditor ? (
        <div className="rounded-xl border border-border bg-surface p-6">
          <h2 className="mb-4 text-lg font-semibold text-slate-100">{asText(toolUi.uploadTitle)}</h2>
          <div
          ...
          >
            ...
            <p className="mb-2 text-sm text-slate-400">
              {isLoading ? asText(uploadUi.loading) : asText(uploadUi.dropOrClick)}
            </p>
            <p className="text-xs text-slate-500">{asText(toolUi.supportedFormats)}</p>
            <input
              ref={fileInputRef}
              type="file"
          ...
          <div className="scrollbar-thin max-h-[70vh] space-y-4 overflow-y-auto pr-2">
            ...
            <div className="rounded-xl border border-border bg-surface p-4">
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label className="mb-1 block text-sm text-slate-400">{asText(filtersUi.contrast)}</label>
                  <input
                    type="range"
                    min="0"
                    max="200"
                ...
                <div>
                  <label className="mb-1 block text-sm text-slate-400">{asText(filtersUi.saturation)}</label>
                  <input
                    type="range"
                    min="0"
                    max="200"
                ...
                <div>
                  <label className="mb-1 flex justify-between text-sm text-slate-400">
                    <span>{asText(filtersUi.warmth)}</span>
                    <span className="text-xs">
                      {warmth < 100
                        ? asText(filtersUi.warmthCool)
                        : warmth > 100
                          ? asText(filtersUi.warmthWarm)
                          : asText(filtersUi.warmthNeutral)}
                    </span>
                  </label>
                  <input
                    type="range"
                ...
                <div>
                  <label className="mb-1 flex justify-between text-sm text-slate-400">
                    <span>{asText(filtersUi.sharpness)}</span>
                    <span className="text-xs">
                      {sharpness < 100
                        ? asText(filtersUi.sharpnessSoft)
                        : sharpness > 100
                          ? asText(filtersUi.sharpnessSharp)
                          : asText(filtersUi.sharpnessNormal)}
                    </span>
                  </label>
                  <input
                    type="range"
                ...
                <div>
                  <label className="mb-1 block text-sm text-slate-400">{asText(filtersUi.exposure)}</label>
                  <input
                    type="range"
                    min="0"
                    max="200"
            ...
            <div className="rounded-xl border border-border bg-surface p-4">
              <label className="mb-4 block border-b border-slate-600 pb-2 font-semibold text-slate-200">
                {asText(sectionsUi.instantEffects)}
              </label>
              <div className="grid grid-cols-3 gap-2">
                {instantEffectKeys.map((effect) => (
                  <button
                    key={effect}
                    type="button"
                    onClick={() =>
                  ...
                  >
                    {asText(effectsUi[effect])}
                  </button>
                ))}
              </div>
              <p className="mt-2 text-xs text-slate-500">{asText(effectsUi.toggleHint)}</p>
            </div>

            {/* Transform */}
            <div className="rounded-xl border border-border bg-surface p-4">
              <label className="mb-4 block border-b border-slate-600 pb-2 font-semibold text-slate-200">
                {asText(sectionsUi.transform)}
              </label>
              <div className="space-y-4">
                <div className="flex gap-2">
                  <button
                    ...
                    title={asText(transformUi.rotateLeft)}
                  >
                  ...
                  <button
                    ...
                    onClick={() => rotate(90)}
                    className="flex flex-1 items-center justify-center rounded-lg bg-blue-600 p-2 text-white hover:bg-blue-500"
                    title={asText(transformUi.rotateRight)}
                  >
                    <svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path
                        strokeLinecap="round"
                  ...
                  <button
                    ...
                    className="flex flex-1 items-center justify-center rounded-lg bg-blue-600 p-2 text-white hover:bg-blue-500"
                    title={asText(transformUi.flipHorizontal)}
                  >
                    <svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path
                        strokeLinecap="round"
                  ...
                  <button
                    ...
                    className="flex flex-1 items-center justify-center rounded-lg bg-blue-600 p-2 text-white hover:bg-blue-500"
                    title={asText(transformUi.flipVertical)}
                  >
                    <svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path
                        strokeLinecap="round"
                ...
                <div>
                  <label className="mb-2 block text-sm text-slate-400">{asText(transformUi.crop)}</label>
                  <div className="flex flex-wrap gap-2">
                    {(["free", "1:1", "4:3", "16:9"] as const).map((r) => (
                      ...
                      >
                        {r === "free" ? asText(transformUi.cropFree) : r}
                      </button>
                    ))}
                  </div>
                  {cropRatio !== "free" && (
                    ...
                    >
                      {asText(transformUi.applyCrop)}
                    </button>
                  )}
                </div>
                <div>
                  <label className="mb-2 block text-sm text-slate-400">{asText(transformUi.resize)}</label>
                  <div className="flex gap-2">
                    <NumberInputWithStepper
                      ...
                      placeholder={asText(transformUi.widthPlaceholder)}
                      min={1}
                      className="w-24"
                    />
                    <NumberInputWithStepper
                      ...
                      placeholder={asText(transformUi.heightPlaceholder)}
                      min={1}
                      className="w-24"
                    />
                    <select
                  ...
                  <label className="mt-2 flex items-center gap-2 text-sm text-slate-400">
                    <input
                      ...
                      className="rounded"
                    />
                    {asText(transformUi.maintainAspect)}
                  </label>
                  <button
                    type="button"
                    onClick={applyResize}
                    className="mt-2 rounded-lg bg-slate-600 px-3 py-1.5 text-sm text-white hover:bg-slate-500"
                  >
                    {asText(transformUi.applyResize)}
                  </button>
                </div>
              </div>
            </div>
            ...
            <div className="rounded-xl border border-border bg-surface p-4">
              <label className="mb-4 block border-b border-slate-600 pb-2 font-semibold text-slate-200">
                {asText(sectionsUi.drawAnnotate)}
              </label>
              <div className="flex gap-2">
                <button
                  type="button"
              ...
              {drawMode && (
                <div className="mt-3 space-y-2 border-t border-slate-600 pt-3">
                  <div className="flex items-center gap-3">
                    <label className="text-xs text-slate-400">{asText(drawUi.color)}</label>
                    <input
                      type="color"
                      value={drawColor}
                    ...
                    />
                    <label className="text-xs text-slate-400">{asText(drawUi.size)}</label>
                    <input
                      type="range"
                      min="1"
                      max="20"
                    ...
                    <span className="text-xs text-slate-500">
                      {formatUi(asText(drawUi.sizePx), { n: drawSize })}
                    </span>
                  </div>
                  <p className="text-xs text-slate-500">
                    {drawMode === "text"
                      ? asText(drawUi.hintText)
                      : asText(drawUi.hintDraw)}
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="rounded-xl border border-border bg-surface p-4">
              <label className="mb-4 block border-b border-slate-600 pb-2 font-semibold text-slate-200">
                {asText(sectionsUi.utilities)}
              </label>
              <div className="space-y-3">
                <label className="flex items-center justify-between">
                  <span className="text-sm text-slate-400">{asText(utilitiesUi.addWatermark)}</span>
                  <button
                    type="button"
                ...
                {watermarkEnabled && (
                  <div className="space-y-3 pt-2">
                    <input
                      type="text"
                      placeholder={asText(utilitiesUi.watermarkPlaceholder)}
                      value={watermarkText}
                      onChange={(e) => setWatermarkText(e.target.value)}
                      className="w-full rounded bg-slate-800 px-3 py-1.5 text-sm text-slate-200 placeholder:text-slate-500"
                    />
                    <div>
                      <label className="mb-1.5 block text-xs text-slate-400">{asText(utilitiesUi.position)}</label>
                      <div className="grid grid-cols-3 gap-1.5">
                        {watermarkPositionOptions.map(({ val, labelKey }) => (
                          <button
                            key={val}
                            type="button"
                          ...
                          >
                            {asText(positionsUi[labelKey])}
                          </button>
                        ))}
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      <label className="text-xs text-slate-400">{asText(utilitiesUi.opacity)}</label>
                      <input
                        type="range"
                        min="0.05"
                        max="1"
                    ...
                    <p className="text-xs text-slate-500">{asText(utilitiesUi.watermarkPreview)}</p>
                  </div>
                )}
                <label className="flex items-center justify-between">
                  <span className="text-sm text-slate-400">{asText(utilitiesUi.compareOriginal)}</span>
                  <button
                    type="button"
                ...
                <p className="text-xs text-slate-500">
                  {compareEnabled
                    ? asText(utilitiesUi.showingOriginal)
                    : asText(utilitiesUi.showingEdited)}
                </p>
              </div>
            </div>
              ...
              <div className="space-y-3">
                <div className="flex gap-2">
                  ...
                  <div className="flex flex-1 flex-col gap-2">
                    ...
                    >
                      <option value="high">{asText(actionsUi.qualityHigh)}</option>
                      <option value="medium">{asText(actionsUi.qualityMedium)}</option>
                      <option value="low">{asText(actionsUi.qualityLow)}</option>
                    </select>
                  </div>
                </div>
              </div>
