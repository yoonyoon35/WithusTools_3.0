...
const META_PATH = "/tools/jpg-converter/pdf";
  ...
  const loadPdf = useCallback(
    async (file: File) => {
      ...
      try {
        ...
        showMessage(err instanceof Error ? err.message : asText(messagesUi.failedLoadPdf), "error");
      } finally {
        setLoading(false);
      ...
      {files.length === 0 ? (
        ...
        >
          ...
          <p className="text-slate-400">
            {asText(toolUi.dropHint)}
          </p>
          <p className="mt-1 text-sm text-slate-500">{asText(toolUi.upTo10Files)}</p>
          <input
            ref={fileInputRef}
            type="file"
            accept=".pdf"
        ...
        <>
          <div className="rounded-xl border border-border bg-surface p-6">
            <h2 className="mb-4 text-lg font-semibold text-slate-100">{asText(toolUi.uploadedPdfs)}</h2>
            <div className="mb-4 flex flex-wrap items-center justify-between gap-4">
              <div className="flex items-center gap-2">
                <span className="text-slate-400">
                  {formatUi(asText(toolUi.fileOf), {
            ...
            <div className="flex flex-wrap items-center gap-4">
                  <div>
                    <label className="mr-2 text-sm text-slate-400">{asText(toolUi.quality)}</label>
                    <input
                      type="range"
                      min={1}
                      max={100}
                  ...
                  >
                    {loading ? asText(toolUi.loading) : asText(toolUi.convertButton)}
                  </button>
              ...
              {!pdfReady ? (
                <p className="py-8 text-center text-slate-400">{asText(toolUi.loadingPdfLibrary)}</p>
              ) : loading ? (
                <div className="py-8">
                  <div className="mb-3 flex items-center justify-between text-sm text-slate-400">
                    <span>
                      {loadProgress?.phase === "loading"
                        ? asText(toolUi.loadingDocument)
                        : loadProgress?.phase === "rendering"
                          ? formatUi(asText(toolUi.renderingPage), {
                              page: loadProgress.page ?? 0,
                              totalPages: loadProgress.totalPages ?? "?",
                            })
                          : asText(toolUi.processing)}
                    </span>
                ...
                >
                  {pagePreviews.map((src, i) => (
                    <div key={i} className="rounded-lg border border-border p-2">
                      ...
                      <img
                        src={src}
                        alt={formatUi(asText(toolUi.page), { n: i + 1 })}
                        className="max-h-48 w-full rounded object-contain"
                      />
                      <p className="mt-2 text-center text-sm text-slate-500">
                        {formatUi(asText(toolUi.page), { n: i + 1 })}
                      </p>
                    </div>
                  ))}
                </div>
            ...
            <div className="mt-6 rounded-xl border border-border bg-surface p-6">
              <div className="mb-4 flex items-center justify-between">
                <h3 className="font-semibold text-slate-100">
                  {formatUi(asText(toolUi.convertedFiles), { name: currentFile?.name ?? "" })}
                </h3>
                <div className="flex gap-2">
                  <button
                  ...
                  <button
                    ...
                    className="rounded border border-border px-4 py-2 text-slate-300"
                  >
                    {currentIndex < files.length - 1 ? asText(toolUi.nextFile) : asText(toolUi.doneReset)}
                  </button>
                </div>
              </div>
              <div
              ...
              >
                {convertedImages.map((dataUrl, i) => (
                  <div key={i} className="rounded-lg border border-border bg-slate-950/50 p-3">
                    ...
                    />
                    <p className="text-xs text-slate-400">{formatUi(asText(toolUi.page), { n: i + 1 })}</p>
                    <button
                      onClick={() =>
                        downloadImage(
                          dataUrl,
