...
export default function JPGConverter({
  ...
  const META_PATH = `/tools/jpg-converter/${format}`;
  ...
  const addFiles = useCallback(
    (fileList: FileList | null) => {
      ...
      if (filtered.length === 0) {
        showMessage(formatUi(asText(messagesUi.selectFormatOnly), { displayName }), "error");
        return;
      }
      setFiles((prev) => [...prev, ...filtered]);
      setConverted([]);
    },
    [format, displayName, showMessage, messagesUi]
  );

  const convertFiles = useCallback(async () => {
    if (files.length === 0) {
      showMessage(asText(messagesUi.noFilesToConvert), "error");
      return;
    }
    setLoading(true);
    setConverted([]);
    ...
    try {
      if (format === "heic" || format === "heif") {
        ...
        const convertTiffRawToJpg = async (file: File): Promise<File> => {
          ...
          try {
            ...
            if (!jpegBlob) throw new Error(formatUi(asText(messagesUi.noEmbeddedJpegPreview), { formatLabel }));
        ...
        const convertCr3ToJpg = async (file: File): Promise<File> => {
          ...
          if (jpegs.length === 0) throw new Error(formatUi(asText(messagesUi.noEmbeddedJpegPreview), { formatLabel: "CR3" }));
          const best = jpegs.reduce((a, b) =>
            b.end - b.start > a.end - a.start ? b : a
          );
          ...
          try {
            const img = await new Promise<HTMLImageElement>((resolve, reject) => {
              const i = new Image();
              i.onload = () => resolve(i);
              i.onerror = () => reject(new Error(asText(messagesUi.failedDecodeEmbedded)));
              i.src = url;
            });
            const canvas = document.createElement("canvas");
            canvas.width = img.naturalWidth;
        ...
        for (let i = 0; i < files.length; i++) {
          ...
          if (!canvas) {
            throw new Error(formatUi(asText(messagesUi.psdReadError), { name: file.name }));
          }
          const blob = await canvasToJpegBlob(canvas, quality / 100);
        ...
        if (!pdfjs) throw new Error(asText(messagesUi.documentLibraryLoading));
        for (let i = 0; i < files.length; i++) {
          updateProgress(i);
          const file = files[i];
          const ab = await file.arrayBuffer();
      ...
      showMessage(asText(messagesUi.conversionCompleted));
    } catch (err) {
      showMessage(err instanceof Error ? err.message : asText(messagesUi.conversionFailed), "error");
      console.error(err);
    } finally {
      setLoading(false);
  ...
  const downloadAll = useCallback(() => {
    if (converted.length === 0) {
      showMessage(asText(messagesUi.noConvertedFiles), "error");
      return;
    }
    converted.forEach(downloadFile);
  }, [converted, downloadFile, showMessage, messagesUi]);
  ...
  const downloadAllAsZip = useCallback(async () => {
    ...
    try {
      ...
      a.click();
      URL.revokeObjectURL(a.href);
      showMessage(asText(messagesUi.zipDownloadStarted));
    } catch (err) {
      showMessage(err instanceof Error ? err.message : asText(messagesUi.zipDownloadFailed), "error");
    }
  }, [converted, showMessage, messagesUi]);
      ...
      <div className="rounded-xl border border-amber-500/45 bg-slate-950 px-5 py-4">
        ...
        {format === "ai" && (
          <p className="mt-3 text-sm font-normal leading-relaxed text-amber-200/85">
            {asText(toolUi.aiNote)}
          </p>
        )}
      </div>

      <div className="rounded-xl border border-slate-700/90 bg-slate-900 p-6 shadow-lg shadow-black/25 sm:p-8">
        <h2 className="mb-6 text-lg font-bold tracking-tight text-white sm:text-xl">{asText(toolUi.uploadTitle)}</h2>
        <div
          role="button"
          tabIndex={0}
          aria-label={formatUi(asText(toolUi.uploadAriaLabel), { displayName })}
          onKeyDown={(e) => {
        ...
        >
          ...
          <p className="mb-2 text-center text-base font-medium text-slate-300">
            {formatUi(asText(toolUi.dropHint), { displayName })}
          </p>
          <p className="text-center text-sm text-slate-500">{asText(toolUi.supportsMultiple)}</p>
          <input
            ref={fileInputRef}
            type="file"
            accept={acceptTypes}
          ...
          <div className="mt-4 space-y-4">
            <div className="mb-4">
              <h3 className="mb-3 text-sm font-medium text-slate-400">
                {formatUi(asText(toolUi.uploadedFiles), { n: files.length })}
              </h3>
              <div
                className={
                  files.length > 10
            ...
            <div className="flex flex-wrap items-center gap-4">
              <div>
                <label className="mb-2 block text-sm text-slate-400">{asText(toolUi.quality)}</label>
              ...
              {hasBackgroundColor && (
                <div>
                  <label className="mb-2 block text-sm text-slate-400">{asText(toolUi.background)}</label>
                  <input
                    type="color"
                    value={bgColor}
              ...
              {hasDimensions && (
                <>
                  <div>
                    <label className="mb-2 block text-sm text-slate-400">{asText(toolUi.width)}</label>
                  ...
                  <div>
                    <label className="mb-2 block text-sm text-slate-400">{asText(toolUi.height)}</label>
                    <NumberInputWithStepper
                      value={String(height)}
              ...
              >
                {loading
                  ? asText(toolUi.converting)
                  : format === "ai" && !pdfReady
                    ? asText(toolUi.loadingLibrary)
                    : asText(toolUi.convertButton)}
              </button>
              <button
                onClick={reset}
            ...
            {loading && convertProgress && (
              <div className="mt-4 space-y-2">
                <div className="flex items-center justify-between text-sm text-slate-400">
                  <span>
                    {formatUi(asText(toolUi.convertProgress), {
                      current: convertProgress.current + 1,
            ...
            {!loading && (
              <p className="mt-4 text-sm text-slate-500">
                {formatUi(asText(toolUi.filesSelected), { n: files.length })}
                {format === "ai" && !pdfReady && asText(toolUi.loadingDocumentLibrary)}
              </p>
            )}
          </div>
        )}
      ...
      {converted.length > 0 && (
        <div className="rounded-xl border border-border bg-surface p-6">
          <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
            <h3 className="font-semibold text-slate-100">{asText(toolUi.convertedFiles)}</h3>
            <div className="flex gap-2">
              <button
                onClick={downloadAll}
                className="rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-500"
              >
                {asText(toolUi.downloadAll)}
              </button>
              <button
          ...
          >
            {converted.map((file, idx) => (
              ...
              >
                ...
                >
                  <span>{asText(toolUi.download)}</span>
                </button>
              </div>
            ))}
          </div>
