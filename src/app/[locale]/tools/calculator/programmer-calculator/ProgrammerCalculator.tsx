  ...
  return (
    ...
    >
      <div className="flex items-center gap-3 border-b border-slate-600/80 px-4 py-3">
        ...
        <h2 className="text-lg font-semibold tracking-tight text-slate-100">{ui.title ?? "Programmer"}</h2>
      </div>
      ...
      <div className="grid grid-cols-6 border-t border-slate-600/60 px-1 py-4 text-xs text-slate-400 sm:px-2">
        <div className="relative flex min-h-[48px] items-center justify-center px-0.5">
          ...
          >
            ...
            <span className="text-sm font-medium text-slate-200 sm:text-base">{ui.keypad ?? "Keypad"}</span>
          </button>
        </div>
        <div className="relative flex min-h-[48px] items-center justify-center px-0.5">
          <button
            ...
            aria-label={ui.bits ?? "Bit toggling"}
        ...
        <div className="relative flex min-h-[48px] items-center justify-center px-0.5">
          <button
            ...
            aria-label={`Shift mode: ${shiftMode === "arithmetic" ? (ui.shiftArithmetic ?? SHIFT_MODE_MENU[shiftMode]) : shiftMode === "logical" ? (ui.shiftLogical ?? SHIFT_MODE_MENU[shiftMode]) : shiftMode === "rotate" ? (ui.shiftRotate ?? SHIFT_MODE_MENU[shiftMode]) : (ui.shiftRotateThroughCarry ?? SHIFT_MODE_MENU[shiftMode])}`}
          >
          ...
          {shiftMenuOpen ? (
            ...
            >
              {SHIFT_MODES_ORDER.map((m) => (
                ...
                >
                  {m === "arithmetic" ? (ui.shiftArithmetic ?? SHIFT_MODE_MENU[m]) : m === "logical" ? (ui.shiftLogical ?? SHIFT_MODE_MENU[m]) : m === "rotate" ? (ui.shiftRotate ?? SHIFT_MODE_MENU[m]) : (ui.shiftRotateThroughCarry ?? SHIFT_MODE_MENU[m])}
                </button>
              ))}
            </div>
          ) : null}
        ...
        >
          {(
            [
              ...
              { label: ui.not ?? "NOT" },
              { label: ui.xor ?? "XOR", op: "^" },
              { label: ui.nor ?? "NOR", op: "nor" },
              { label: ui.nand ?? "NAND", op: "nand" },
            ] as const
          ).map((btn) => (
            <button
              key={btn.label}
        ...
        <div className="grid grid-cols-5 gap-1.5 p-3 sm:gap-2" style={{ backgroundColor: "rgb(15 23 42 / 0.5)" }}>
          {(["A", "<<", ">>", "C", "⌫"] as const).map((label, i) => {
            ...
            return (
              ...
              >
                {label === "⌫" ? (ui.backspace ?? "⌫") : label === "C" ? (ui.clear ?? "C") : label}
              </button>
            );
          })}
