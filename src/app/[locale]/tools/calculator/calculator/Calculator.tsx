      ...
      <div className="rounded-xl border border-border bg-surface p-4">
        <div className="mb-4 rounded-lg border border-slate-600 bg-slate-900/50 p-4 text-right">
          <div className="flex min-h-[1.25rem] items-center justify-between text-sm text-slate-500">
            <span>{hasMemory && <span className="rounded bg-slate-600 px-1.5 py-0.5 font-medium text-slate-300" title={`${ui.memoryIndicator}: ${formatDisplay(formatForStorage(memory))}`}>M</span>}</span>
            <span>
          ...
          <div className="col-span-4 flex gap-2">
            {(["mc", "mr", "m+", "m-", "ms"] as const).map((a) => (
              <button key={a} type="button" className={`${btnMemory} min-w-0 flex-1`} onClick={{ mc: memoryClear, mr: memoryRecall, "m+": memoryAdd, "m-": memorySubtract, ms: memoryStore }[a]}>
                {a === "mc" ? ui.memoryClear : a === "mr" ? ui.memoryRecall : a === "m+" ? ui.memoryAdd : a === "m-" ? ui.memorySubtract : ui.memoryStore}
              </button>
            ))}
          </div>
          {/* Rows 2–7: 3 content + 1 operator */}
            <button type="button" className={btnFunction} onClick={percent}>{ui.percent}</button>
            <button type="button" className={btnFunction} onClick={() => setCurrentOperand("0")}>{ui.clearEntry}</button>
            <button type="button" className={btnFunction} onClick={reset}>{ui.clearAll}</button>
            <button type="button" className={btnFunction} onClick={deleteLast}>{ui.backspace}</button>
            <button type="button" className={btnFunction} onClick={reciprocal}>{ui.reciprocal}</button>
            <button type="button" className={btnFunction} onClick={square}>{ui.square}</button>
            <button type="button" className={btnFunction} onClick={sqrt}>{ui.sqrt}</button>
            ...
            <button type="button" className={btnFunction} onClick={negate}>{ui.negate}</button>
            ...
            <button type="button" className={btnEquals} onClick={calculate}>{ui.equals}</button>
        </div>
      </div>
    </div>
  );
