...
const META_PATH = "/tools/calculator/average-calculator";
...
export default function AverageCalculator() {
  ...
  const runCalculate = useCallback(() => {
    ...
    if (!numbers || numbers.length === 0) {
      if (input.trim()) setError(ui?.errorInvalidNumbers ?? "Please enter valid numbers");
      setResults(null);
      return;
    }
    const stats = calculateStatistics(numbers, useSampleVariance);
    setResults(stats);
  }, [input, parseAndValidate, ui, useSampleVariance]);
  ...
  useEffect(() => {
    ...
    const tid = setTimeout(() => {
      ...
      if (numbers && numbers.length > 0) {
        ...
        setResults(null);
        setError(ui?.errorInvalidNumbers ?? "Please enter valid numbers");
      }
    }, 300);
      ...
      <div className="rounded-xl border border-border bg-surface p-6">
        ...
        <div>
          ...
          <textarea
            ...
            rows={4}
            placeholder={ui.numbersPlaceholder}
        ...
        <div className="mt-4 flex flex-wrap items-center gap-3">
          ...
          {results && (
            ...
            >
              {copyFeedback ? ui.copied : ui.copyResults}
            </button>
          )}
        </div>
        ...
        <div className="mt-4 flex flex-wrap items-center gap-4 border-t border-slate-700 pt-4">
          ...
          <div className="flex items-center gap-2">
            <span className="text-sm text-slate-400">{ui.variance}:</span>
            <select
            ...
            >
              <option value="sample">{ui.varianceSample}</option>
              <option value="population">{ui.variancePopulation}</option>
            </select>
          </div>
          <label className="flex cursor-pointer items-center gap-2">
            ...
            />
            <span className="text-sm text-slate-400">{ui.thousandsSeparator}</span>
          </label>
        </div>
        ...
        {results && (
          ...
          >
            ...
            <div className="mt-6">
              <p className="mb-2 text-sm text-slate-400">{ui.sortedData}</p>
