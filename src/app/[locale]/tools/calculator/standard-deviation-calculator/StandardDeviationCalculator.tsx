...
export default function StandardDeviationCalculator() {
  ...
  const reset = () => {
    ...
    setResults(null);
    setError("");
    showToast(ui?.toastCleared ?? "Cleared");
  };

  const loadSample = () => {
    setInput(SAMPLE_DATA);
    showToast(ui?.toastSampleLoaded ?? "Sample data loaded", "info");
  };

  const sortInputAscending = () => {
    const numbers = parseNumbers(input);
    if (!numbers || numbers.length === 0) {
      showToast(ui?.toastEnterValidFirst ?? "Enter valid numbers first", "info");
      return;
    }
    const sorted = [...numbers].sort((a, b) => a - b);
    setInput(sorted.join(", "));
    showToast(ui?.toastSorted ?? "Sorted ascending", "info");
  };
