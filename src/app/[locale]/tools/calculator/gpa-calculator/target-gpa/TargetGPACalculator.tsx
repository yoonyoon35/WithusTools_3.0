...
export default function TargetGPACalculator() {
  ...
  if (T === null || T < 0 || T > scaleMax) {
    message = ui?.messageTargetRange ?? "Enter a target GPA within your grading scale range.";
  } else if (G === null || G < 0 || G > scaleMax) {
    message = ui?.messageCurrentGpa ?? "Enter a valid current cumulative GPA.";
  } else if (C === null || C < 0) {
    message = ui?.messageCredits ?? "Enter valid cumulative credits (0 or more).";
  } else if (N === null || N <= 0) {
    message =
      ui?.messageTermCredits ??
  ...
  const copyResults = async () => {
    if (requiredTermGpa === null) {
      showToast(ui?.nothingToCopy ?? "Nothing to copy yet");
      return;
    }
    const lines = [
    ...
    try {
      ...
      showToast(ui?.copiedToClipboard ?? "Copied to clipboard");
    } catch {
      showToast(ui?.copyFailed ?? "Copy failed");
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className={cardCls}>
        <h3 className="mb-4 text-lg font-semibold text-slate-200">{ui.settings}</h3>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder={ui.expectedTermPlaceholder}
        ...
        >
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            <div className="text-center">
              <p className="text-xs text-slate-500">{ui.requiredTermGpa}</p>
              <p
            ...
            <div className="text-center">
              <p className="text-xs text-slate-500">{ui.approxLetter}</p>
              <p className="mt-1 text-xl font-semibold text-slate-100">{letterApprox}</p>
            </div>
            <div className="text-center">
              <p className="text-xs text-slate-500">{ui.creditsAtExpected}</p>
