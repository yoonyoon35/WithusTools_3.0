...
const META_PATH = "/tools/time/pomodoro";
...
export default function Pomodoro() {
  ...
  const getPhaseConfig = useCallback(
    (p: Phase) => {
      ...
      switch (p) {
        case "focus":
          return { duration: DEFAULT_WORK, label: asText(uiNow.phaseFocus), color: "text-emerald-500" };
        case "shortBreak":
          return { duration: DEFAULT_SHORT_BREAK, label: asText(uiNow.phaseShortBreak), color: "text-blue-500" };
        case "longBreak":
          return { duration: DEFAULT_LONG_BREAK, label: asText(uiNow.phaseLongBreak), color: "text-amber-500" };
      }
    },
    []
  );
  ...
  const tick = useCallback(() => {
    ...
    if (ms <= 0) {
      ...
      if ("Notification" in window && Notification.permission === "granted") {
        ...
        new Notification(formatUi(asText(uiNow.notificationTitle), { phase: config.label }), {
          body:
            phase === "focus"
              ? asText(uiNow.notificationBreakBody)
              : asText(uiNow.notificationFocusBody),
          icon: "/favicon.ico",
      ...
      if (phase === "focus") {
        ...
        setModalMessage(asText(uiNow.breakMessage));
        ...
        setModalMessage(asText(uiNow.focusMessage));
        setAutoAdvanceCountdown(AUTO_ADVANCE_SECONDS);
        setModalOpen(true);
        advanceTimeoutRef.current = setTimeout(() => {
          setModalOpen(false);
  ...
  const autoAdvanceUnit =
    autoAdvanceCountdown === 1 ? asText(toolUi.second) : asText(toolUi.seconds);
    ...
    <div className="mx-auto max-w-2xl">
      ...
      >
        ...
        <p className="text-sm text-slate-500">
          {phase === "focus"
            ? formatUi(asText(toolUi.sessionOf), {
                current: (pomodoroCount % POMODOROS_BEFORE_LONG) + 1,
                total: POMODOROS_BEFORE_LONG,
              })
            : asText(toolUi.takeBreak)}
        </p>
      </div>
      ...
      <div className="mb-6 flex justify-center gap-4">
        ...
        <button
          onClick={reset}
          className="flex h-14 w-14 items-center justify-center rounded-full bg-slate-600 text-white transition-transform hover:scale-105 hover:bg-slate-500"
          title={asText(toolUi.resetTitle)}
        >
          <svg className="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path
              strokeLinecap="round"
        ...
        <button
          ...
          title={soundOn ? asText(toolUi.muteTitle) : asText(toolUi.unmuteTitle)}
        >
          {soundOn ? (
      ...
      <div className="mb-4 rounded-lg border border-slate-700 bg-slate-800/50 px-4 py-3 text-center text-sm text-slate-400">
        {formatUi(asText(toolUi.rhythmInfo), {
          focus: 25,
          shortBreak: 5,
          longBreak: 15,
          count: POMODOROS_BEFORE_LONG,
        })}
      </div>
      ...
      {modalOpen && (
        ...
        >
          ...
          >
            ...
            <h2 className="mb-2 text-2xl font-bold text-slate-100">{asText(toolUi.phaseCompleteTitle)}</h2>
            <p className="mb-2 text-slate-400">{modalMessage}</p>
            <p className="mb-6 text-sm text-slate-500">
              {formatUi(asText(toolUi.autoAdvance), { n: autoAdvanceCountdown, unit: autoAdvanceUnit })}
            </p>
