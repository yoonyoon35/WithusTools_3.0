...
const META_PATH = "/tools/time/timer";
...
function asPresets(value: unknown): { label: string; unit: string }[] {
  if (!Array.isArray(value)) return [];
  return value.map((item) => {
    const m = asMap(item);
    return { label: asText(m.label), unit: asText(m.unit) };
  });
}
...
export default function Timer() {
  ...
  const presetLabels = asPresets(toolUi.presets);
  ...
  const tick = useCallback(() => {
    ...
    if (remaining <= 0) {
      ...
      if ("Notification" in window && Notification.permission === "granted") {
        ...
        new Notification(asText(uiNow.notificationTitle) || "Timer Complete!", {
          body: asText(uiNow.notificationBody) || "The set time has ended.",
          icon: "/favicon.ico",
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      <div className="mb-6 flex flex-wrap justify-center gap-3">
        {PRESETS.map((p, i) => {
          const preset = presetLabels[i];
          const label = preset?.label || String(p.time / 60);
          const unit = preset?.unit || asText(toolUi.presetMin);
          return (
            <button
              key={p.time}
      ...
      {modalOpen && (
        ...
        >
          ...
          >
            ...
            <h2 className="mb-2 text-2xl font-bold text-slate-100">{asText(toolUi.timesUpTitle)}</h2>
            <p className="mb-6 text-slate-400">{asText(toolUi.timesUpMessage)}</p>
            <button
              onClick={reset}
