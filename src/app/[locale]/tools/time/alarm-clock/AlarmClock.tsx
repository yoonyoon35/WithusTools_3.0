...
const META_PATH = "/tools/time/alarm-clock";
...
function formatTime12(time24: string, am: string, pm: string) {
...
export default function AlarmClock() {
  ...
  const toolUiRef = useRef(toolUi);

  useEffect(() => {
    toolUiRef.current = toolUi;
  }, [toolUi]);

  useEffect(() => {
    const name = asText(toolUi.defaultAlarmName);
    if (name) setAlarmName(name);
  }, [toolUi.defaultAlarmName]);
        ...
        if (updated) {
          ...
          if (triggered) {
            ...
            if ("Notification" in window && Notification.permission === "granted") {
              const uiNow = toolUiRef.current;
              const am = asText(uiNow.am) || "AM";
              const pm = asText(uiNow.pm) || "PM";
              const timeStr = formatTime12(triggered.time, am, pm);
              new Notification(triggered.name, {
                body: formatUi(asText(uiNow.notificationBody) || "Time: {time}", { time: timeStr }),
                icon: "/favicon.ico",
  ...
  const defaultAlarmName = asText(toolUi.defaultAlarmName) || "My Alarm";
  const amLabel = asText(toolUi.am) || "AM";
  const pmLabel = asText(toolUi.pm) || "PM";
  ...
  const addAlarm = () => {
    ...
    const name = alarmName.trim() || defaultAlarmName;
    if (alarms.some((a) => a.time === time && a.name === name)) {
      alert(asText(toolUi.duplicateAlert) || "An alarm with the same time and name already exists.");
      return;
    }
  ...
  const deleteAlarm = (id: number) => {
    if (confirm(asText(toolUi.deleteConfirm) || "Are you sure you want to delete this alarm?")) {
  ...
  const saveEdit = () => {
    ...
    setAlarms((p) =>
      p.map((a) =>
        a.id === editingId ? { ...a, name: editName.trim() || defaultAlarmName, time } : a
      )
    );
    setEditingId(null);
  };
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      <div className="mb-6 rounded-xl border border-border bg-surface p-6">
        <h3 className="mb-4 text-lg font-semibold text-slate-200">{asText(toolUi.addAlarmTitle)}</h3>
        <div className="space-y-4">
          <div>
            <label className="mb-1 block text-sm text-slate-400">{asText(toolUi.alarmNameLabel)}</label>
            <input
              type="text"
              value={alarmName}
              onChange={(e) => setAlarmName(e.target.value)}
              placeholder={asText(toolUi.alarmNamePlaceholder)}
              maxLength={50}
          ...
          <div>
            <label className="mb-2 block text-sm text-slate-400">{asText(toolUi.timeLabel)}</label>
          ...
          >
            ...
            {asText(toolUi.addAlarmButton)}
          </button>
        </div>
      </div>

      <div className="mb-6 rounded-xl border border-border bg-surface p-6">
        <h3 className="mb-4 text-lg font-semibold text-slate-200">{asText(toolUi.alarmsListTitle)}</h3>
        {sortedAlarms.length === 0 ? (
          <div className="py-12 text-center text-slate-500">
            ...
            <p className="font-medium">{asText(toolUi.noAlarmsTitle)}</p>
            <p className="text-sm">{asText(toolUi.noAlarmsHint)}</p>
          </div>
        ) : (
            ...
            {sortedAlarms.map((alarm) => (
              ...
              >
                ...
                >
                  ...
                  <div className="font-mono text-slate-400">{formatTime12(alarm.time, amLabel, pmLabel)}</div>
                </div>
                <div className="flex gap-2">
                  <button
                    ...
                    title={alarm.active ? asText(toolUi.disableTitle) : asText(toolUi.enableTitle)}
                  >
                    <svg
                  ...
                  <button
                    ...
                    title={asText(toolUi.deleteTitle)}
                  >
                    <svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path
                        strokeLinecap="round"
      ...
      {editingId !== null && (
        ...
        >
          ...
          >
            <h3 className="mb-4 text-lg font-semibold text-slate-200">{asText(toolUi.editAlarmTitle)}</h3>
            <div className="space-y-4">
              <div>
                <label className="mb-1 block text-sm text-slate-400">{asText(toolUi.alarmNameLabel)}</label>
                <input
        ...
        >
          ...
          >
            ...
            <p className="mb-6 text-slate-400">
              {asText(toolUi.modalTimePrefix)} {formatTime12(modalAlarm.time, amLabel, pmLabel)}
            </p>
            <button
              onClick={dismissModal}
              className="rounded-lg bg-blue-600 px-6 py-3 font-medium text-white hover:bg-blue-700"
            >
              {asText(toolUi.stopAlarm)}
            </button>
          </div>
        </div>
      )}
