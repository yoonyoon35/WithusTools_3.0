...
const META_PATH = "/tools/time/stopwatch";
...
export default function Stopwatch() {
  ...
  const buildExportContent = (records: LapRecord[]) => {
    const header = asText(toolUi.exportHeader);
    const totalLine = formatUi(asText(toolUi.exportTotalTime), { time: formatTime(elapsed) });
    const lapTimesHeader = asText(toolUi.exportLapTimes);
    let content = `${header}\n${totalLine}\n${lapTimesHeader}\n`;
    records.forEach((r) => {
      const lapLine = formatUi(asText(toolUi.exportLapLine), { n: r.lapNum });
      const lapTimeLine = formatUi(asText(toolUi.exportLapTime), { time: r.lapTime });
      const totalTimeLine = formatUi(asText(toolUi.exportTotalTimeLine), { time: r.totalTime });
      content += `${lapLine}\n  ${lapTimeLine}\n  ${totalTimeLine}\n\n`;
    });
    return content;
  };
  ...
  const exportToText = () => {
    const defaultName = asText(toolUi.exportDefaultName) || "Stopwatch Records";
    const name = prompt(asText(toolUi.exportPrompt), defaultName)?.trim() || defaultName;
    const records = [...laps].reverse();
    const content = buildExportContent(records);
    ...
    <div className="mx-auto max-w-2xl">
      ...
      <div className="mb-6 rounded-xl border border-border bg-surface p-6">
        <div className="mb-4 flex items-center justify-between">
          <h3 className="text-lg font-semibold text-slate-200">{asText(toolUi.lapTimesTitle)}</h3>
          <div className="flex gap-2">
            <button
              onClick={exportToWord}
              className="rounded-lg bg-slate-600 px-3 py-1.5 text-sm text-slate-200 hover:bg-slate-500"
            >
              {asText(toolUi.wordExport)}
            </button>
            <button
              onClick={exportToText}
              className="rounded-lg bg-slate-600 px-3 py-1.5 text-sm text-slate-200 hover:bg-slate-500"
            >
              {asText(toolUi.textExport)}
            </button>
          </div>
        </div>
        <div className="max-h-64 space-y-2 overflow-y-auto">
          {laps.length === 0 ? (
            <p className="py-8 text-center text-slate-500">{asText(toolUi.noLaps)}</p>
          ) : (
            laps.map((lap) => (
              <div
                key={lap.lapNum}
              ...
              >
                <span className="text-slate-400">{formatUi(asText(toolUi.lapLabel), { n: lap.lapNum })}</span>
