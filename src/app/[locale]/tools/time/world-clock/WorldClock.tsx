...
const META_PATH = "/tools/time/world-clock";
...
function getUiTimezones(toolUi: ReturnType<typeof asMap>) {
  if (!Array.isArray(toolUi.timezones)) return null;
  return toolUi.timezones.map((item) => {
    const m = asMap(item);
    return {
      id: asText(m.id),
      city: asText(m.city),
...
function mergeTimeZones(toolUi: ReturnType<typeof asMap>) {
  const uiZones = getUiTimezones(toolUi);
  if (!uiZones) return TIME_ZONES;
  return TIME_ZONES.map((tz, i) => {
    const uiZone = uiZones.find((z) => z.id === tz.id && z.city === tz.city) || uiZones[i];
    if (!uiZone) return tz;
    return {
      ...tz,
      city: uiZone.city || tz.city,
      label: uiZone.label || tz.label,
    };
  });
}
...
function formatDateInZone(date: Date, timeZone: string, locale: string) {
  return new Intl.DateTimeFormat(locale, {
    timeZone,
    weekday: "short",
    month: "short",
...
export default function WorldClock() {
  ...
  const timeZones = mergeTimeZones(toolUi);
  ...
  return (
    <div className="mx-auto max-w-5xl overflow-x-hidden">
      <div className="mb-6 rounded-xl border border-border bg-surface p-6">
        <h3 className="mb-4 text-lg font-semibold text-slate-200">{asText(toolUi.worldClocksTitle)}</h3>
        <p className="mb-4 text-sm text-slate-500">{asText(toolUi.hoverHint)}</p>
                ...
                {timeZones.map((tz) => (
                  ...
                  >
                    ...
                    <span className="font-mono text-base text-slate-100" suppressHydrationWarning>
                      {formatInZone(now, tz.id, locale)}
                    </span>
                  </div>
                ))}
              </div>
              {timeZones.length > VISIBLE_COUNT && (
                <p className="mt-2 text-xs text-slate-500">
                  {formatUi(asText(toolUi.scrollMore), { n: timeZones.length - VISIBLE_COUNT })}
                </p>
              )}
            </div>
          ) : (
            <div className="flex shrink-0 items-center lg:w-[400px]">
              <p className="text-slate-500">{asText(toolUi.loading)}</p>
            </div>
          )}
      ...
      <div className="mb-6 rounded-xl border border-border bg-surface p-6">
        <h3 className="mb-4 text-lg font-semibold text-slate-200">{asText(toolUi.converterTitle)}</h3>
        <p className="mb-4 text-sm text-slate-500">{asText(toolUi.converterIntro)}</p>
        <div className="space-y-4">
          <div className="flex flex-wrap gap-4">
            <div>
              <label className="mb-1 block text-xs text-slate-500">{asText(toolUi.dateLabel)}</label>
              <input
                type="date"
                value={converterDate}
            ...
            <div>
              <label className="mb-1 block text-xs text-slate-500">{asText(toolUi.time24hLabel)}</label>
            ...
            <div>
              <label className="mb-1 block text-xs text-slate-500">{asText(toolUi.timezoneLabel)}</label>
              <select
                value={fromZone}
              ...
              >
                {timeZones.map((tz) => (
                  <option key={`${tz.id}-${tz.city}`} value={tz.id}>
                    {tz.city} ({tz.label})
                  </option>
                ))}
              </select>
          ...
          <div>
            <span className="text-xs text-slate-500">{asText(toolUi.useCurrentTime)}</span>
            <button
              onClick={() => {
                if (now) {
                  ...
                  const found = timeZones.find((t) => t.id === tz)?.id || timeZones[0].id;
                  setFromZone(found);
                }
              }}
              className="ml-2 rounded bg-slate-600 px-2 py-1 text-xs text-white hover:bg-slate-500"
            >
              {asText(toolUi.nowButton)}
            </button>
          </div>
        </div>

        <div className="mt-6 border-t border-slate-700 pt-4">
          <h4 className="mb-3 text-sm font-medium text-slate-300">{asText(toolUi.convertedTimesTitle)}</h4>
          {convMoment && (
              ...
              {[...timeZones]
                ...
                >
                  ...
                  <span className="font-mono text-slate-100" suppressHydrationWarning>
                    {formatInZone(convMoment, tz.id, locale)} {formatDateInZone(convMoment, tz.id, locale)}
                  </span>
                </div>
              ))}
            </div>
