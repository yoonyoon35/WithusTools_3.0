...
const META_PATH = "/tools/time/date-difference";
...
export default function DateDifference() {
  ...
  const ddayTodayLabel = asText(toolUi.ddayToday);
  const ddayLabel = asText(toolUi.dday);
      ...
      ? (() => {
          ...
          if (diffMs > 0) {
            ...
            return {
              ...
              seconds,
              totalHours,
              label: formatUi(asText(toolUi.ddayFuture), { n: calendarDays }),
              isFuture: true,
            };
          }
          ...
          return {
            ...
            totalHours,
            label:
              calendarDaysPassed === 0
                ? ddayLabel
                : formatUi(asText(toolUi.ddayPast), { n: calendarDaysPassed }),
            isFuture: false,
          };
        })()
      : null;

  const dayUnit = (n: number) => (n !== 1 ? asText(toolUi.days) : asText(toolUi.day));
  ...
  return (
    <div className="mx-auto max-w-2xl">
      <div className="rounded-xl border border-border bg-surface p-6">
        <h3 className="mb-4 text-lg font-semibold text-slate-200">{asText(toolUi.calculateTitle)}</h3>

        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            <label htmlFor="startDate" className="mb-1 block text-sm text-slate-400">
              {asText(toolUi.startDateLabel)}
            </label>
            <input
              id="startDate"
              type="date"
          ...
          <div className="mt-6 space-y-4">
            {ddayLive && (
              ...
              >
                ...
                {ddayLive.label !== ddayTodayLabel && (
                  <p className="mt-6 text-2xl text-slate-400 sm:text-3xl">
                    {[
                      ddayLive.days > 0 && `${ddayLive.days} ${dayUnit(ddayLive.days)}`,
                      `${ddayLive.hours}h ${ddayLive.minutes}m ${ddayLive.seconds}s`,
                    ]
                      .filter(Boolean)
                      .join(" ")}
                    {ddayLive.isFuture ? asText(toolUi.left) : asText(toolUi.ago)}
                  </p>
                )}
              </div>
            )}
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
              <div className="rounded-lg border border-slate-600 bg-slate-800/50 p-4 text-center">
                <p className="text-xs text-slate-500">{asText(toolUi.hoursLabel)}</p>
                <p className="text-xl font-semibold text-slate-100">
                  {(ddayLive && "totalHours" in ddayLive
                    ? (ddayLive.isFuture ? 1 : -1) * ddayLive.totalHours
                    : result.hours
                  ).toLocaleString()}{" "}
                  {asText(toolUi.hoursUnit)}
                </p>
              </div>
              <div className="rounded-lg border border-slate-600 bg-slate-800/50 p-4 text-center">
                <p className="text-xs text-slate-500">{asText(toolUi.daysLabel)}</p>
                <p className="text-xl font-semibold text-slate-100">
                  {result.days.toLocaleString()} {asText(toolUi.daysUnit)}
                </p>
              </div>
              <div className="rounded-lg border border-slate-600 bg-slate-800/50 p-4 text-center">
                <p className="text-xs text-slate-500">{asText(toolUi.weeksLabel)}</p>
                <p className="text-xl font-semibold text-slate-100">
                  {result.weeks.toFixed(1)} {asText(toolUi.weeksUnit)}
                </p>
              </div>
              <div className="rounded-lg border border-slate-600 bg-slate-800/50 p-4 text-center">
                <p className="text-xs text-slate-500">{asText(toolUi.monthsLabel)}</p>
                <p className="text-xl font-semibold text-slate-100">
                  {result.months} {asText(toolUi.monthsUnit)}
                </p>
                {result.extraDays !== 0 && (
                  <p className="mt-1 text-sm text-slate-400">
                    {formatUi(asText(toolUi.extraDays), {
                      sign: result.extraDays > 0 ? "+" : "",
                      n: result.extraDays,
                    })}
                  </p>
                )}
              ...
              <div className="rounded-lg border border-slate-600 bg-slate-800/50 p-4 text-center sm:col-span-2 sm:col-start-2">
                <p className="text-xs text-slate-500">{asText(toolUi.directionLabel)}</p>
                <p className="text-xl font-semibold text-slate-100">
                  {result.days >= 0 ? (
                    <span className="text-emerald-400">
                      {formatUi(asText(toolUi.daysToFuture), { n: result.days })}
                    </span>
                  ) : (
                    <span className="text-amber-400">
                      {formatUi(asText(toolUi.daysAgoDirection), { n: Math.abs(result.days) })}
                    </span>
                  )}
                </p>
              </div>
        ...
        {!result && endDate && (
          <p className="mt-4 text-sm text-amber-400">{asText(toolUi.invalidDates)}</p>
        )}
      </div>
    </div>
  );
