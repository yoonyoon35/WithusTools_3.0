...
const META_PATH = "/tools/time/interval-timer";
...
export default function IntervalTimer() {
  ...
  const buildOneRound = useCallback(() => {
    ...
    const restLabel = asText(uiNow.rest) || "Rest";
    const actionTemplate = asText(uiNow.actionDefault) || "Action {n}";
    ...
    actions.forEach((a, i) => {
      seq.push({
        ...
        index: i,
        duration: a.duration,
        label: a.name.trim() || formatUi(actionTemplate, { n: i + 1 }),
        colorId: a.colorId,
      });
      if (restEnabled && i < actions.length - 1) {
        seq.push({ type: "rest", duration: restDuration, label: restLabel, colorId: "rest" });
      }
    });
    return seq;
      ...
      if (repeatEnabledRef.current) {
        ...
        if (maxRounds === null || roundRef.current < maxRounds) {
          if (restEnabledRef.current && restDurationRef.current > 0 && (maxRounds === null || maxRounds >= 2)) {
            ...
            setCurrentLabel(asText(toolUiRef.current.rest) || "Rest");
  ...
  const tick = useCallback(() => {
    ...
    if (ms <= 0) {
      ...
      if ("Notification" in window && Notification.permission === "granted") {
        const uiNow = toolUiRef.current;
        new Notification(asText(uiNow.notificationTitle) || "Interval Timer", {
          body: formatUi(asText(uiNow.notificationBody) || "{label} complete!", {
            label: item?.label || asText(uiNow.phaseDefault) || "Phase",
          }),
          icon: "/favicon.ico",
  ...
  const saveRoutine = () => {
    const name = window.prompt(asText(toolUi.routineNamePrompt) || "Routine name?");
    if (!name?.trim()) return;
    ...
    <div className="mx-auto max-w-2xl">
      ...
      {!running && remaining === 0 && !routineComplete && (
        <div className="mb-6 space-y-4 rounded-xl border border-border bg-surface p-6">
          <h3 className="text-lg font-semibold text-slate-200">{asText(toolUi.actionsTitle)}</h3>
          <p className="text-sm text-slate-500">{asText(toolUi.actionsIntro)}</p>
          <div className="space-y-4">
            {actions.map((a) => (
              <div key={a.id} className="flex flex-wrap items-center gap-3 rounded-lg border border-slate-700 bg-slate-800/50 p-4">
                <input
                  type="text"
                  placeholder={asText(toolUi.actionNamePlaceholder)}
                  value={a.name}
                ...
                <NumberInputWithStepper
                  ...
                  className="w-28"
                />
                <span className="text-slate-500">{asText(toolUi.sec)}</span>
          ...
          <div className="mt-4 space-y-3 border-t border-slate-700 pt-4">
            <label className="flex items-center gap-3">
              ...
              />
              <span className="text-slate-300">{asText(toolUi.restBetweenActions)}</span>
            </label>
            {restEnabled && (
            ...
            <label className="flex items-center gap-3">
              <input
                ...
                className="h-4 w-4 rounded border-slate-600 accent-emerald-500"
              />
              <span className="text-slate-300">{asText(toolUi.repeatRoutine)}</span>
            </label>
            {repeatEnabled && (
              <div className="flex items-center gap-2 pl-7">
                <NumberInputWithStepper
                  ...
                  min={1}
                  placeholder={asText(toolUi.repeatPlaceholder)}
                  className="w-32"
                />
                <span className="text-slate-500">{asText(toolUi.repeatHint)}</span>
              </div>
            )}
          </div>
        </div>
        ...
        {running || remaining > 0 || routineComplete ? (
          <>
            <p className={`mb-2 text-lg font-semibold`} style={{ color: colorConfig.value }}>
              {routineComplete ? asText(toolUi.routineComplete) : currentLabel}
            </p>
            {repeatEnabled && (repeatCount !== "" || actions.length > 0) && (
              <p className="mb-1 text-xs text-slate-500">
                {formatUi(asText(toolUi.round), { n: currentRound })}
              </p>
            )}
          ...
          <div className="text-left">
            <h3 className="mb-4 text-lg font-semibold text-slate-200">{asText(toolUi.savedRoutinesTitle)}</h3>
            {savedRoutines.length === 0 ? (
              <p className="text-slate-500">{asText(toolUi.noSavedRoutines)}</p>
            ) : (
      ...
      <div className="mb-6 flex justify-center gap-4">
        ...
        <button
          ...
          title={running ? asText(toolUi.pauseTitle) : asText(toolUi.startTitle)}
        >
          {running ? (
