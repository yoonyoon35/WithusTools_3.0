...
const META_PATH = "/tools/text/string-comparison";
...
export default function StringComparison() {
  ...
  const compare = useCallback(() => {
    if (!ui) return;
    if (!text1 || !text2) {
      showToast(ui.toastEnterBoth, "error");
      return;
    }
    ...
    showToast(
      count === 1
        ? formatToolUiString(ui.toastDiffFoundOne, { count })
        : formatToolUiString(ui.toastDiffFoundMany, { count })
    );
  }, [text1, text2, trim, ignoreWhitespace, ignoreCase, showToast, ui]);
  ...
  const copyText = useCallback(
    (text: string) => {
      ...
      navigator.clipboard
        ...
        .catch(() => showToast(ui.toastCopyFailed, "error"));
    },
    [showToast, ui]
  );
    ...
    (e: React.ChangeEvent<HTMLInputElement>, setText: (value: string) => void) => {
      ...
      reader.onload = () => {
        ...
        if (typeof result === "string") {
          setText(result);
          showToast(formatToolUiString(ui.toastLoaded, { name: file.name }));
        } else {
          showToast(ui.toastReadBinaryFailed, "error");
        }
      };
      reader.onerror = () => showToast(ui.toastReadFailed, "error");
    ...
    <div className="space-y-6">
      <div className="grid gap-6 md:grid-cols-2">
        <div className="rounded-xl border border-border bg-surface p-6">
          <h2 className="mb-3 text-lg font-semibold text-slate-100">{ui.firstText}</h2>
          <textarea
            id="text1"
            value={text1}
            onChange={(e) => setText1(e.target.value)}
            placeholder={ui.firstPlaceholder}
        ...
        <div className="rounded-xl border border-border bg-surface p-6">
          <h2 className="mb-3 text-lg font-semibold text-slate-100">{ui.secondText}</h2>
          <textarea
            id="text2"
            value={text2}
            onChange={(e) => setText2(e.target.value)}
            placeholder={ui.secondPlaceholder}
            className="scrollbar-thin max-h-[50vh] w-full resize-none overflow-y-auto rounded-lg border border-border bg-slate-950 px-4 py-3 pr-2 text-slate-100 placeholder-slate-500 focus:border-slate-600 focus:outline-none focus:ring-1 focus:ring-slate-600"
            rows={6}
          />
          <div className="mt-3 flex flex-wrap gap-2">
      ...
      <div className="rounded-xl border border-border bg-surface p-6">
        <h2 className="mb-3 text-lg font-semibold text-slate-100">{ui.comparisonOptions}</h2>
      ...
      {(html1 || html2) && (() => {
        ...
        return (
          <div className="rounded-xl border border-border bg-surface p-6">
            <h2 className="mb-3 text-lg font-semibold text-slate-100">{ui.comparisonResult}</h2>
            <div className="grid gap-6 md:grid-cols-2">
              <div>
                <p className="mb-2 text-sm text-slate-400">{ui.firstResult}</p>
                <div ref={scrollRef1} className={resultScrollClass}>
                  <LineNumberedContent html={html1} />
                </div>
              </div>
              <div>
                <p className="mb-2 text-sm text-slate-400">{ui.secondResult}</p>
      ...
      <div className="rounded-xl border border-border bg-surface p-6">
        <h2 className="mb-3 text-lg font-semibold text-slate-100">{ui.statistics}</h2>
        <div className="grid gap-6 sm:grid-cols-2">
          <div>
            <p className="mb-2 text-sm font-medium text-slate-400">{ui.firstText}</p>
            <div className="flex flex-wrap gap-4 text-sm">
              <span className="text-slate-300">
                {ui.characters}: <span className="font-semibold text-slate-100">{getTextStats(text1).chars}</span>
              </span>
              <span className="text-slate-300">
                {ui.words}: <span className="font-semibold text-slate-100">{getTextStats(text1).words}</span>
              </span>
              <span className="text-slate-300">
                {ui.lines}: <span className="font-semibold text-slate-100">{getTextStats(text1).lines}</span>
              </span>
            </div>
          </div>
          <div>
            <p className="mb-2 text-sm font-medium text-slate-400">{ui.secondText}</p>
            <div className="flex flex-wrap gap-4 text-sm">
              <span className="text-slate-300">
                {ui.characters}: <span className="font-semibold text-slate-100">{getTextStats(text2).chars}</span>
              </span>
              <span className="text-slate-300">
                {ui.words}: <span className="font-semibold text-slate-100">{getTextStats(text2).words}</span>
              </span>
              <span className="text-slate-300">
                {ui.lines}: <span className="font-semibold text-slate-100">{getTextStats(text2).lines}</span>
              </span>
            </div>
          </div>
        </div>
        {diffCount !== null && (
          <div className="mt-4 space-y-3 border-t border-border pt-4">
            <div className="flex flex-wrap items-center gap-4">
              <span className="text-slate-300">
                {ui.differences}: <span className="font-semibold text-slate-100">{diffCount}</span>
              </span>
              <span
              ...
              >
                {diffCount === 0 ? ui.same : ui.different}
              </span>
            </div>
            {diffDetails.length > 0 && (
              <div className="space-y-1.5 text-sm">
                <p className="text-slate-400">{ui.differenceDetails}</p>
                <div
                  className={
                  ...
                  {diffDetails.map((d, i) => (
                    <div key={i} className="rounded-lg border border-border bg-slate-950/50 px-3 py-2 font-mono">
                      <span className="text-slate-500">
                        {formatToolUiString(ui.lineLabel, { line: d.line })}
                      </span>{" "}
