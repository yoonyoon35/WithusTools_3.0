...
const META_PATH = "/tools/text/text-editor";
...
export default function TextEditor() {
  ...
  const saveDocument = useCallback(() => {
    ...
    setTitle("");
    setContent("");
    showToast(ui.toastSaved);
  }, [title, content, documents, showToast, ui]);
  ...
  const deleteNote = useCallback(
    (id: number) => {
      if (!ui) return;
      if (!confirm(ui.toastDeleteConfirm)) return;
      const next = documents.filter((d) => d.id !== id);
      setDocuments(next);
      localStorage.setItem("documents", JSON.stringify(next));
      showToast(ui.toastDeleted);
    },
    [documents, showToast, ui]
  );
    ...
    (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      reader.onload = () => {
        ...
        if (typeof result === "string") {
          ...
          showToast(ui.toastReadBinaryFailed);
        }
      };
      reader.onerror = () => showToast(ui.toastReadFailed);
  ...
  const copyContent = useCallback(() => {
    ...
    if (content) {
      navigator.clipboard
        ...
        .catch(() => showToast(ui.toastCopyFailed));
    } else {
      showToast(ui.toastNoContent);
    }
  }, [content, showToast, ui]);
  ...
  const exportToTxt = useCallback(() => {
    ...
    showToast(ui.toastExportedTxt);
  }, [title, content, showToast, ui]);
  ...
  const exportToWord = useCallback(() => {
    ...
    a.click();
    URL.revokeObjectURL(url);
    showToast(ui.toastExportedWord);
  }, [title, content, showToast, ui]);
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-xl border border-border bg-surface p-6">
        <h2 className="mb-4 text-lg font-semibold text-slate-100">{ui.document}</h2>
        <input
          type="text"
          value={title}
        ...
        <div className="mb-4 flex flex-wrap items-center justify-between gap-4 rounded-lg bg-slate-950/80 px-4 py-2 text-sm">
          <span className="text-slate-400">
            {ui.characters}: <span className="font-medium text-slate-200">{charCount}</span>
          </span>
          <span className="text-slate-400">
            {ui.words}: <span className="font-medium text-slate-200">{wordCount}</span>
          </span>
        </div>
      ...
      <div className="rounded-xl border border-border bg-surface p-6">
        <h2 className="mb-4 text-lg font-semibold text-slate-100">{ui.savedNotes}</h2>
