...
const META_PATH = "/tools/seo/favicon-generator";
...
export default function FaviconGenerator() {
  ...
  const handleFile = useCallback(
    (file: File) => {
      if (!file.type.startsWith("image/")) {
        showMessage(asText(messagesUi.selectImageFile), "error");
        return;
      }
      const reader = new FileReader();
      reader.onerror = () => showMessage(asText(messagesUi.failedReadFile), "error");
      reader.onload = (e) => {
        const dataUrl = e.target?.result as string;
        if (!dataUrl) return;
        const img = new Image();
        img.onerror = () => showMessage(asText(messagesUi.failedLoadImage), "error");
        img.onload = () => {
          if (img.width < 260 || img.height < 260) {
            showMessage(asText(messagesUi.imageTooSmall), "error");
            return;
          }
  ...
  const generate = useCallback(async () => {
    ...
    try {
      ...
      const siteNameVal = siteName.trim() || asText(toolUi.siteNamePlaceholder) || "Your Site Name";
      const shortNameVal = shortName.trim() || asText(toolUi.shortNamePlaceholder) || "Site";
...
<browserconfig>
    <msapplication>
      ...
      showMessage(asText(messagesUi.failedGenerate), "error");
    }
  }, [previewUrl, basic, apple, android, windows, safari, bgColor, transparentBg, siteName, shortName, iconPath, createFavicon, showMessage, messagesUi, toolUi]);
...
<browserconfig>
    <msapplication>
      ...
      showMessage(asText(messagesUi.packageDownloaded));
    } catch (err) {
      showMessage(asText(messagesUi.failedDownload), "error");
    }
  }, [previewUrl, basic, apple, android, windows, safari, bgColor, transparentBg, siteName, shortName, iconPath, createFavicon, showMessage, messagesUi, toolUi]);
  ...
  const platformOptions = [
    { id: "basic", label: asText(toolUi.platformBasic), checked: basic, set: setBasic },
    { id: "apple", label: asText(toolUi.platformApple), checked: apple, set: setApple },
    { id: "android", label: asText(toolUi.platformAndroid), checked: android, set: setAndroid },
    { id: "windows", label: asText(toolUi.platformWindows), checked: windows, set: setWindows },
    { id: "safari", label: asText(toolUi.platformSafari), checked: safari, set: setSafari },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-xl border border-border bg-surface p-6">
        <h2 className="mb-4 text-lg font-semibold text-slate-100">{asText(toolUi.uploadImage)}</h2>
        <input
          ref={fileInputRef}
          id="favicon-file"
          type="file"
        ...
        {!previewUrl ? (
          <div
            ...
            onDrop={(e) => {
              ...
              if (!f) {
                showMessage(asText(messagesUi.pleaseDropImage), "error");
                return;
              }
              if (!f.type.startsWith("image/")) {
                showMessage(asText(messagesUi.pleaseDropImageType), "error");
                return;
              }
              handleFile(f);
            }}
          ...
          >
            ...
            <p className="mb-2 text-sm text-slate-400">{asText(toolUi.dropImageHere)}</p>
            <p className="text-xs text-slate-500">{asText(toolUi.recommendedSize)}</p>
          </div>
        ) : (
          <div className="flex items-center gap-4">
            <img src={previewUrl} alt={asText(toolUi.previewAlt)} className="h-24 w-24 rounded-lg object-cover" />
      ...
      <div className="rounded-xl border border-border bg-surface p-6">
        <h3 className="mb-4 font-semibold text-slate-200">{asText(toolUi.options)}</h3>
        <div className="space-y-4">
          <div className="space-y-2">
            <p className="text-sm text-slate-400">{asText(toolUi.platforms)}</p>
            <div className="flex flex-wrap gap-4">
              {platformOptions.map(({ id, label, checked, set }) => (
          ...
          <div>
            <span className="block text-sm text-slate-400">{asText(toolUi.backgroundLabel)}</span>
            <label className="mt-2 flex cursor-pointer items-center gap-2 text-sm text-slate-300">
              ...
              />
              {asText(toolUi.noBackground)}
            </label>
            <p className="mt-1 text-xs text-slate-500">
              {transparentBg ? asText(toolUi.transparentHintOn) : asText(toolUi.transparentHintOff)}
            </p>
            <label className="mt-3 block text-sm text-slate-400">
              {transparentBg ? asText(toolUi.themeColor) : asText(toolUi.fillColor)}
            </label>
            <input
              type="color"
              value={bgColor}
          ...
          <div>
            <label className="block text-sm text-slate-400">{asText(toolUi.siteName)}</label>
            <input
              type="text"
              value={siteName}
              onChange={(e) => setSiteName(e.target.value)}
              placeholder={asText(toolUi.siteNamePlaceholder)}
              className="mt-1 w-full rounded-lg border border-slate-600 bg-slate-800 px-3 py-2 text-slate-200"
            />
          </div>
          <div>
            <label className="block text-sm text-slate-400">{asText(toolUi.shortName)}</label>
            <input
              type="text"
              value={shortName}
              onChange={(e) => setShortName(e.target.value)}
              placeholder={asText(toolUi.shortNamePlaceholder)}
              className="mt-1 w-full rounded-lg border border-slate-600 bg-slate-800 px-3 py-2 text-slate-200"
            />
          </div>
          <div>
            <label className="block text-sm text-slate-400">{asText(toolUi.faviconOutputPath)}</label>
            <input
              type="text"
              value={iconPath}
              onChange={(e) => setIconPath(e.target.value)}
              placeholder={asText(toolUi.faviconOutputPathPlaceholder)}
              className="mt-1 w-full rounded-lg border border-slate-600 bg-slate-800 px-3 py-2 text-slate-200"
            />
          </div>
          <button
      ...
      {results && (
        <div className="rounded-xl border border-border bg-surface p-6">
          <h3 className="mb-4 font-semibold text-slate-200">{asText(toolUi.generatedFavicons)}</h3>
          ...
          <p className="mb-6 text-xs text-slate-500">
            {asText(toolUi.previewDisclaimer)}
          </p>
          <div className="space-y-4">
            <div>
              <p className="mb-2 text-sm font-medium text-slate-300">{asText(toolUi.htmlCode)}</p>
              <div className="relative">
                ...
                >
                  {copiedId === "html" ? asText(toolUi.copied) : asText(toolUi.copy)}
                </button>
              </div>
            </div>
            <div>
              <p className="mb-2 text-sm font-medium text-slate-300">{asText(toolUi.siteWebmanifest)}</p>
              <div className="relative">
                <pre className="scrollbar-thin max-h-[50vh] overflow-y-auto rounded-lg bg-slate-900 p-4 pr-2 text-sm text-slate-300">
                  {manifestCode}
                </pre>
                <button
                  ...
                  className="absolute right-2 top-2 rounded bg-slate-700 px-2 py-1 text-xs hover:bg-slate-600"
                >
                  {copiedId === "manifest" ? asText(toolUi.copied) : asText(toolUi.copy)}
                </button>
              </div>
            </div>
            <div>
              <p className="mb-2 text-sm font-medium text-slate-300">{asText(toolUi.browserconfigXml)}</p>
              <div className="relative">
                <pre className="scrollbar-thin max-h-[50vh] overflow-y-auto rounded-lg bg-slate-900 p-4 pr-2 text-sm text-slate-300">
                  {browserconfigCode}
                </pre>
                <button
                  ...
                  className="absolute right-2 top-2 rounded bg-slate-700 px-2 py-1 text-xs hover:bg-slate-600"
                >
                  {copiedId === "browserconfig" ? asText(toolUi.copied) : asText(toolUi.copy)}
                </button>
              </div>
            </div>
          </div>
