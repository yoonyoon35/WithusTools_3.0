...
const META_PATH = "/tools/seo/sitemap-generator";
const MISSING_URL_SENTINEL = "__MISSING_WEBSITE_URL__";
...
export default function SitemapGenerator() {
  ...
  const generateSitemap = useCallback(() => {
    ...
    if (!baseUrl) return MISSING_URL_SENTINEL;
...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
  ...
  const hasValidSitemap = websiteUrl.trim().length > 0 && xmlOutput !== MISSING_URL_SENTINEL;
  ...
  const updateUrl = (id: string, field: keyof UrlItem, value: string) => {
    setUrlItems((prev) => prev.map((u) => (u.id === id ? { ...u, [field]: value } : u)));
  };

  const copyXml = () => {
    if (!hasValidSitemap) {
      showMessage(asText(messagesUi.enterWebsiteUrl), "error");
      return;
    }
    navigator.clipboard
      .writeText(xmlOutput)
  ...
  const displayXml =
    xmlOutput === MISSING_URL_SENTINEL ? asText(messagesUi.missingWebsiteUrl) : xmlOutput;
    ...
    <div className="grid gap-6 lg:grid-cols-2">
      ...
      <div className="space-y-6 rounded-xl border border-border bg-surface p-6">
        <div className="rounded-lg border border-slate-700/80 bg-slate-800/30 p-4">
          <label className="block text-sm font-medium text-slate-300">{asText(toolUi.websiteUrl)}</label>
          <input
            type="url"
            value={websiteUrl}
            onChange={(e) => setWebsiteUrl(e.target.value)}
            placeholder={asText(toolUi.websitePlaceholder)}
          ...
          <div className="mt-3 flex flex-wrap items-center gap-3">
            <div className="flex items-center gap-2">
              <label className="text-xs text-slate-500">{asText(toolUi.lastModified)}</label>
              <input
                type="date"
                value={baseLastmod}
            ...
            <span className="text-xs text-slate-600">{asText(toolUi.emptyUrlHint)}</span>
          </div>
        </div>

        <div>
          <div className="mb-3 flex items-center justify-between">
            <label className="text-sm font-medium text-slate-300">{asText(toolUi.urlList)}</label>
            <button
              type="button"
              onClick={addUrl}
                  ...
                  <div className="min-w-0 flex-1 space-y-3">
                    <div>
                      <input
                        ...
                        placeholder={asText(toolUi.urlPlaceholder)}
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                      <div>
                        <label className="mb-0.5 block text-xs text-slate-500">
                          {asText(toolUi.lastModified)}
                        </label>
                        <input
                          type="date"
                          value={item.lastmod}
                          onChange={(e) => updateUrl(item.id, "lastmod", e.target.value)}
                          title={asText(toolUi.emptyUrlHint)}
                      ...
                      <div>
                        ...
                        >
                          <option value="always">{asText(toolUi.freqAlways)}</option>
                          <option value="hourly">{asText(toolUi.freqHourly)}</option>
                          <option value="daily">{asText(toolUi.freqDaily)}</option>
                          <option value="weekly">{asText(toolUi.freqWeekly)}</option>
                          <option value="monthly">{asText(toolUi.freqMonthly)}</option>
                          <option value="yearly">{asText(toolUi.freqYearly)}</option>
                          <option value="never">{asText(toolUi.freqNever)}</option>
                        </select>
                      </div>
                      <div>
                        <label className="mb-0.5 block text-xs text-slate-500">
                          {asText(toolUi.priority)}
                        </label>
      ...
      <div className="rounded-xl border border-border bg-surface p-6">
        <div className="mb-4 flex items-center justify-between">
          <h3 className="font-semibold text-slate-200">{asText(toolUi.generatedSitemap)}</h3>
