...
const HUB_META_PATH = "/tools/hash-calculator";

function mapErrorMessage(message: string, ui: Record<string, string>): string {
  const known: Record<string, string | undefined> = {
    "Enter text to hash.": ui.errorEnterText,
    "Hash generation failed": ui.errorHashGenFailed,
    "Failed to read file": ui.errorFileReadFailed,
    "File hash failed": ui.errorFileHashFailed,
    "Enter password and hash to verify.": ui.errorEnterPasswordHash,
    "Enter password and Argon2 hash to verify.": ui.errorEnterPasswordArgon2,
    "Enter password, salt, and derived key (hex) to verify.":
      ui.errorEnterPasswordSaltKey,
    "Verification failed": ui.errorVerificationFailed,
    "Failed to copy": ui.errorCopyFailed,
  };
...
export default function HashCalculator({
  ...
  const hubContent = useToolPageContent(HUB_META_PATH);
  const pageContent = useToolPageContent(`/tools/hash-calculator/${algorithm}`);
  const ui = hubContent?.ui;
  ...
  const handleFile = useCallback(
    async (file: File) => {
      ...
      try {
        ...
        const raw = err instanceof Error ? err.message : ui.errorFileHashFailed;
        setError(mapErrorMessage(raw, ui));
      } finally {
        setLoading(false);
      }
    },
    [algorithm, isSha3, sha3Variant, isXxHash, xxHashVariant, isFNV1a, fnv1aVariant, isBlake2, blake2Variant, ui]
  );
  ...
  const verify = useCallback(async () => {
    ...
    if (isBcrypt) {
      ...
      if (!pwd || !h) {
        setError(ui.errorEnterPasswordHash);
        return;
      }
      setError("");
      setVerifyResult(null);
      try {
        ...
        const raw = err instanceof Error ? err.message : ui.errorVerificationFailed;
        setError(mapErrorMessage(raw, ui));
      ...
      const h = verifyHashInput.trim();
      if (!pwd || !h) {
        setError(ui.errorEnterPasswordArgon2);
        return;
      }
      setError("");
      setVerifyResult(null);
      ...
      if (!pwd || !salt || !keyHex) {
        setError(ui.errorEnterPasswordSaltKey);
        return;
      }
      setError("");
      setVerifyResult(null);
  ...
  }, [isBcrypt, isArgon2, isPBKDF2, isScrypt, verifyInput, verifyHashInput, verifyPbkdf2Salt, verifyPbkdf2SaltFormat, verifyPbkdf2Iterations, verifyPbkdf2Hash, verifyScryptSalt, verifyScryptSaltFormat, verifyScryptN, verifyScryptR, verifyScryptP, hash, ui]);
  ...
  useEffect(() => {
    if (showPageHeader && pageContent?.h1) {
      document.title = `${pageContent.h1} | WithusTools`;
    }
  }, [showPageHeader, pageContent?.h1]);

  if (!ui) return null;

  const isPasswordHasher = isBcrypt || isArgon2 || isPBKDF2 || isScrypt;

  return (
    <div className="space-y-6">
      {showPageHeader && pageContent && (
        <div className="mb-8 flex flex-col items-center justify-center gap-4">
          <div className="text-center">
            <h1 className="text-3xl font-bold text-slate-100">{pageContent.h1}</h1>
            <p className="mt-1 text-sm text-slate-500">{pageContent.subtitle}</p>
          </div>
        </div>
      )}
      ...
      {isPBKDF2 && (
        <details className="rounded-xl border border-blue-500/30 bg-blue-500/5">
          <summary className="cursor-pointer px-4 py-3 font-medium text-blue-400 hover:text-blue-300">
            {ui.pbkdf2GuideSummary}
          </summary>
          <div className="space-y-3 px-4 pb-4 pt-1 text-sm text-slate-300">
            <div>
              <strong className="text-slate-200">{ui.pbkdf2SaltTitle}</strong>
              <ul className="mt-1 list-inside list-disc space-y-0.5 text-slate-400">
                <li>{ui.pbkdf2SaltHex}</li>
                <li>{ui.pbkdf2SaltUtf8}</li>
              </ul>
            </div>
            <div>
              <strong className="text-slate-200">{ui.pbkdf2OutputTitle}</strong>
              <p className="mt-1 text-slate-400">{ui.pbkdf2OutputBody}</p>
            </div>
            <div>
              <strong className="text-slate-200">{ui.pbkdf2VerifyTitle}</strong>
              <p className="mt-1 text-slate-400">{ui.pbkdf2VerifyBody}</p>
            </div>
            <div>
              <strong className="text-slate-200">{ui.pbkdf2CompareTitle}</strong>
              <p className="mt-1 text-slate-400">{ui.pbkdf2CompareBody}</p>
            </div>
          </div>
        </details>
      )}
      ...
      <div className="rounded-xl border border-border bg-surface p-6">
        <h2 className="mb-2 text-lg font-semibold text-slate-100">
          {isPasswordHasher ? ui.passwordInput : ui.textInput}
        </h2>
        <div className="relative">
          <input
            ...
            placeholder={
              isPasswordHasher ? ui.placeholderPassword : ui.placeholderText
            }
            ...
            <div>
              ...
              <div className="flex gap-2">
                <input
                  ...
                  placeholder={ui.saltCrossSitePlaceholder}
            ...
            <div>
              ...
              <div className="flex gap-2">
                <input
                  ...
                  placeholder={ui.saltPlainHexPlaceholder}
                  className="min-w-0 flex-1 rounded-lg border border-border bg-slate-950 px-4 py-2 text-slate-100 placeholder:text-slate-500 focus:border-blue-500 focus:outline-none"
                />
                <button
                  type="button"
                ...
                <select
                  ...
                  title={ui.saltFormatAutoTitle}
                >
                  {SALT_FORMAT_OPTIONS.map((o) => (
                    <option key={o.value} value={o.value}>
                      {ui.saltFormatPrefix} {o.label}
                    </option>
                  ))}
                </select>
              </div>
        ...
        >
          {loading
            ? ui.processing
            : formatToolUiString(ui.generateHash, { name: displayName })}
        </button>
      </div>

      {supportsFile && !isPasswordHasher && (
        <div className="rounded-xl border border-border bg-surface p-6">
          <h2 className="mb-2 text-lg font-semibold text-slate-100">
            {ui.fileInput}
          </h2>
      ...
      {hash && (
        <div className="rounded-xl border border-border bg-surface p-6">
          <div className="mb-2 flex items-center justify-between">
            <h3 className="font-semibold text-slate-100">{ui.generatedHash}</h3>
            <button
          ...
          {usedParams.length > 0 && (
            <div className="mt-3 rounded-lg bg-slate-950/80 p-3 text-sm">
              <div className="mb-2 font-medium text-slate-400">{ui.parametersUsed}</div>
      ...
      {supportsVerify && (hash || isBcrypt || isArgon2 || isPBKDF2 || isScrypt) && (
        <div className="rounded-xl border border-border bg-surface p-6">
          <h3 className="mb-3 font-semibold text-slate-100">
            {isPasswordHasher ? ui.verifyPassword : ui.hashVerification}
          </h3>
          <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center">
            {isPasswordHasher ? (
              <div className="flex w-full flex-col gap-3">
                <div className="flex flex-wrap gap-3 sm:flex-nowrap">
                  <input
                    ...
                    placeholder={ui.placeholderVerifyPassword}
                  ...
                  <input
                    ...
                    placeholder={
                      isPBKDF2 || isScrypt
                        ? ui.placeholderDerivedKey
                        : isArgon2
                          ? ui.placeholderArgon2Hash
                          : ui.placeholderBcryptHash
                    }
                ...
                {isPBKDF2 && (
                  <div className="flex flex-wrap gap-3">
                    <input
                      ...
                      placeholder={ui.placeholderSaltPlainHex}
              ...
              <input
                ...
                value={verifyInput}
                onChange={(e) => setVerifyInput(e.target.value)}
                placeholder={ui.placeholderCompareHash}
          ...
          {verifyResult !== null && (
            <div className="mt-3 flex items-center gap-2">
              {verifyResult ? (
                <span className="text-green-400">{ui.match}</span>
              ) : (
                <span className="text-red-400">{ui.noMatch}</span>
              )}
            </div>
          )}
        </div>
      ...
      {showAlgorithmGuide && pageContent && (
        <ToolPageGuide
          title={pageContent.guideTitle}
          intro={pageContent.guideIntro}
      ...
      <div className="flex flex-wrap gap-2">
        ...
        >
          {pageContent?.backToHub ?? ui.backToHub}
        </Link>
        <Link
          href="/"
          className="inline-block text-slate-400 underline transition-colors hover:text-slate-200"
