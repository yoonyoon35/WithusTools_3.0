...
const META_PATH = "/tools/gif-converter/pdf";
const MAX_PAGES = 60;
const MIN_FRAMES = 2;
...
export default function PdfToAnimatedGif() {
  ...
  const displayName = asText(page?.displayName) || "PDF";
  ...
  const loadPdfPages = useCallback(
    async (file: File) => {
      ...
      if (!pdfjs) {
        showMessage(asText(messagesUi.pdfLibraryLoading), "error");
        return;
      }
      setLoadingPdf(true);
      setLoadPercent(0);
      ...
      try {
        ...
        if (pdf.numPages > MAX_PAGES) {
          showMessage(
            formatUi(asText(messagesUi.pageLimit), { max: MAX_PAGES, total: pdf.numPages }),
            "error"
          );
        }
        ...
        showMessage(formatUi(asText(messagesUi.loadedPages), { n: canvases.length }));
      } catch (e) {
        showMessage(e instanceof Error ? e.message : asText(messagesUi.failedLoadPdf), "error");
      } finally {
        setLoadingPdf(false);
        setLoadPercent(0);
      }
  ...
  const pickPdf = useCallback(
    (list: FileList | null) => {
      ...
      if (!pdfs.length) {
        showMessage(asText(messagesUi.selectPdfOnly), "error");
        return;
      }
      if (pdfs.length > 1) {
        showMessage(asText(messagesUi.firstPdfOnly), "error");
      }
      setPdfFile(pdfs[0]);
    },
    [showMessage, messagesUi]
  ...
  const reset = useCallback(() => {
    ...
    showMessage(asText(messagesUi.resetComplete));
  }, [showMessage, messagesUi]);

  const createAnimatedGif = useCallback(async () => {
    if (pageCanvases.length < MIN_FRAMES) {
      showMessage(formatUi(asText(messagesUi.minPages), { min: MIN_FRAMES }), "error");
      return;
    }
    ...
    try {
      ...
      for (const pageCanvas of pageCanvases) {
        ...
        canvas.height = h;
        const ctx = canvas.getContext("2d");
        if (!ctx) throw new Error(asText(messagesUi.canvasUnavailable));
      ...
      showMessage(e instanceof Error ? e.message : asText(messagesUi.failedCreateGif), "error");
    } finally {
      setEncoding(false);
    }
  }, [pageCanvases, outWidth, outHeight, quality, frameDelayMs, bgColor, loopForever, showMessage, messagesUi]);
      ...
      {!pdfFile ? (
        <div className="rounded-xl border border-slate-700/90 bg-slate-900 p-6 shadow-lg shadow-black/25 sm:p-8">
          ...
          <div
            ...
            aria-label={asText(toolUi.uploadAriaLabel)}
          ...
          >
            ...
            <p className="text-center text-sm text-slate-500">
              {formatUi(asText(toolUi.pdfHint), { max: MAX_PAGES })}
            </p>
          </div>
        </div>
      ) : (
        <div className="space-y-6">
          <div className="rounded-xl border border-slate-700/90 bg-slate-900 p-6 shadow-lg shadow-black/25 sm:p-8">
              ...
              <p className="text-sm text-slate-300">
                ...
                {loadingPdf ? (
                  <span className="ml-2 text-amber-400">
                    {formatUi(asText(toolUi.loadingPages), { percent: loadPercent })}
                  </span>
                ) : (
                  <span className="ml-2 text-slate-500">
                    {formatUi(asText(toolUi.pageCount), { n: pageCanvases.length })}
                  </span>
                )}
              </p>
              <button
                ...
                {pagePreviews.map((src, idx) => (
                  <div key={idx} className="rounded-lg border border-slate-700 bg-slate-950/50 p-3">
                    <span className="text-xs font-medium text-slate-500">
                      {formatUi(asText(toolUi.pageLabel), { n: idx + 1 })}
                    </span>
            ...
            <div className="grid gap-6 border-t border-slate-700 pt-6 lg:grid-cols-2">
              <div>
                <label className="mb-2 block text-sm font-medium text-slate-300">{asText(toolUi.frameDelay)}</label>
              ...
              <div>
                <label className="mb-2 block text-sm font-medium text-slate-300">{asText(toolUi.gifQuality)}</label>
                <div className="flex items-center gap-3">
                  <input
                    type="range"
                    min={1}
              ...
              <div>
                <label className="mb-2 block text-sm font-medium text-slate-300">{asText(toolUi.outputWidth)}</label>
                <NumberInputWithStepper
              ...
              <div>
                <label className="mb-2 block text-sm font-medium text-slate-300">{asText(toolUi.outputHeight)}</label>
                <NumberInputWithStepper
              ...
              <div>
                <label className="mb-2 block text-sm font-medium text-slate-300">{asText(toolUi.letterboxBg)}</label>
                <input
                  type="color"
                  value={bgColor}
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              ...
              >
                {encoding
                  ? formatUi(asText(toolUi.encoding), { percent: encodeProgress })
                  : asText(toolUi.createButton)}
              </button>
              <button
                type="button"
                onClick={downloadGif}
            ...
            {encoding && (
              <div className="mt-4">
                <div className="mb-1 flex justify-between text-xs text-slate-500">
                  <span>{asText(toolUi.encodingPages)}</span>
          ...
          {resultUrl && resultBlob && (
            <div className="rounded-xl border border-slate-700/90 bg-slate-900 p-6 shadow-lg shadow-black/25 sm:p-8">
              <h3 className="mb-3 text-lg font-semibold text-slate-100">{asText(toolUi.preview)}</h3>
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={resultUrl} alt={asText(toolUi.previewAlt)} className="max-h-96 rounded border border-slate-700 bg-slate-950 object-contain" />
              <p className="mt-2 text-sm text-slate-500">
                {formatUi(asText(toolUi.fileSize), { size: formatSize(resultBlob.size) })}
              </p>
            </div>
          )}
        </div>
