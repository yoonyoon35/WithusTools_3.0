...
const META_PATH = "/tools/gif-converter";

const GIF_PATHS = GIF_CONVERTER_FORMATS.map(
  (format) => `/tools/gif-converter/${format}`
) as readonly string[];
...
export default async function GIFConverterIndexPage({
  params,
}: {
  params: { locale: string };
}) {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      ...
      {content.introNoteBefore ? (
        <p className="mx-auto mb-8 max-w-2xl text-center text-sm text-slate-500">
          {content.introNoteBefore}{" "}
          <Link
            href="/tools/image/images-to-animated-gif"
            className="text-blue-400 underline hover:text-blue-300"
          >
            {ui?.imagesToAnimatedGifLink ?? "Images to Animated GIF"}
          </Link>{" "}
          {content.introNoteAfter}
        </p>
      ) : null}

      <HubToolGrid paths={GIF_PATHS} />
