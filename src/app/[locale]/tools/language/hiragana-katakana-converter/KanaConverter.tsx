...
const META_PATH = "/tools/language/hiragana-katakana-converter";
    ...
    <div className="w-full space-y-6">
      ...
      <div className="rounded-lg border border-border bg-surface p-4">
        ...
        <h2 id="kana-outputs-heading" className="text-lg font-semibold text-slate-200">
          {asText(toolUi.output)}
        </h2>
        <div className="grid gap-6 sm:grid-cols-2">
          <div className="flex flex-col gap-2">
            <h3 className="text-base font-medium text-slate-200">{asText(toolUi.hiraganaHeading)}</h3>
            <textarea
              value={hiragana}
              readOnly
              rows={6}
              aria-label={asText(toolUi.hiraganaAriaLabel)}
          ...
          <div className="flex flex-col gap-2">
            <h3 className="text-base font-medium text-slate-200">{asText(toolUi.katakanaHeading)}</h3>
            <textarea
              value={katakana}
              readOnly
              rows={6}
              aria-label={asText(toolUi.katakanaAriaLabel)}
              className="scrollbar-thin w-full resize-none rounded-lg border border-border bg-slate-950 px-4 py-3 text-slate-300"
            />
            <button
              type="button"
