...
const META_PATH = "/tools/language/alphabet-study";
...
const ALPHABET_ROWS: [number, number][] = [
  [0, 5],
  [5, 5],
  [10, 5],
  [15, 5],
...
export default function AlphabetStudy() {
  ...
  const letterNames = asMap(toolUi.letterNames);
  const writingTips = asMap(toolUi.writingTips);
  const speechLang = asText(toolUi.speechLang) || "en-US";
  const practiceModes = asMap(toolUi.practiceModes);
  const practiceSectionHeaders = asMap(toolUi.practiceSectionHeaders);

  const resolveLetterName = useCallback(
    (char: string, fallback: string) => asText(letterNames[char]) || fallback,
    [letterNames]
  );

  const resolveWritingTip = useCallback(
    (char: string, kind: "tip" | "lowerTip", fallback: string) => {
      const tips = asMap(writingTips[char]);
      return asText(tips[kind]) || fallback;
    },
    [writingTips]
  );
  ...
  const displayTip = selectedItem
    ? viewMode === "upper"
      ? resolveWritingTip(selectedItem.char, "tip", selectedItem.tip)
      : resolveWritingTip(selectedItem.char, "lowerTip", selectedItem.lowerTip)
    : null;
  ...
  const handleReplay = useCallback(() => {
    if (selectedItem) {
      setReplayKey((k) => k + 1);
      speak(resolveLetterName(selectedItem.char, selectedItem.name));
    }
  }, [selectedItem, speak, resolveLetterName]);

  const buildPracticeSheetHTML = useCallback(() => {
    const items = ALPHABET_DATA.map((item) => ({
      char: item.char,
      lower: item.lower,
      name: resolveLetterName(item.char, item.name),
    }));
    ...
    if (practiceSheetMode === "upper") {
      tbody = sectionHeader(asText(practiceSectionHeaders.upper)) + upperRows.join("");
    } else if (practiceSheetMode === "lower") {
      tbody = sectionHeader(asText(practiceSectionHeaders.lower)) + lowerRows.join("");
    } else {
      tbody =
        sectionHeader(asText(practiceSectionHeaders.upper)) +
        upperRows.join("") +
        sectionHeader(asText(practiceSectionHeaders.lower)) +
        lowerRows.join("");
    }
  ...
  }, [practiceSheetMode, practiceSectionHeaders.lower, practiceSectionHeaders.upper, resolveLetterName]);

  const handlePrint = useCallback(() => {
    const tableHTML = buildPracticeSheetHTML();
    const printWindow = window.open("", "_blank");
    if (!printWindow) {
      alert(asText(toolUi.popupBlocked));
      return;
    }
    const printTitle = asText(toolUi.printTitle);
    printWindow.document.write(`
      ...
      *{print-color-adjust:exact;-webkit-print-color-adjust:exact}
      ...
      <body><h1 style="font-size:1.1rem;margin-bottom:.5rem">${printTitle}</h1>${tableHTML}</body>
      </html>`);
  ...
  }, [buildPracticeSheetHTML, toolUi.popupBlocked, toolUi.printTitle]);

  const formatStrokeCount = (n: number) =>
    formatUi(asText(n === 1 ? toolUi.strokeCountOne : toolUi.strokeCountOther), { n });
    ...
    <div className="w-full space-y-6">
      <div className="flex flex-col items-center gap-6 lg:flex-row lg:justify-center lg:items-start">
        <div className="flex-shrink-0 rounded-xl border border-border bg-surface p-6 text-center">
          <h2 className="mb-4 border-b-2 border-blue-500 pb-2 text-lg font-semibold text-slate-100">
            {asText(toolUi.fullListHeading)}
          </h2>
          <div className="space-y-6">
            {ALPHABET_ROWS.map(([start, count], rowIndex) => (
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="text-sm text-slate-500">
                          {resolveLetterName(item.char, item.name)}
                        </span>
                      </li>
                    );
                  })}
        ...
        <div className="sticky top-24 flex-shrink-0 rounded-xl border border-border bg-surface p-6 lg:w-[280px]">
          <h2 className="mb-4 border-b-2 border-blue-500 pb-2 text-lg font-semibold text-slate-100">
            {asText(toolUi.letterDetails)}
          </h2>
          {!selectedItem ? (
            <p className="text-slate-500">{asText(toolUi.clickPrompt)}</p>
          ) : (
            <div className="flex flex-col items-center gap-3">
              ...
              <div className="text-lg text-slate-500">
                {resolveLetterName(selectedItem.char, selectedItem.name)}
              </div>
              <div className="w-full rounded-lg bg-background p-3">
                <h3 className="mb-1 text-sm font-medium text-slate-200">{asText(toolUi.strokeOrder)}</h3>
                {viewMode === "upper" ? (
                  <>
                    <span className="mb-2 block text-sm text-slate-500">({formatStrokeCount(displayStrokes)})</span>
                    ...
                    >
                      ...
                      {asText(toolUi.replay)}
                    </button>
                  </>
                ) : (
                  <p className="text-sm text-slate-500">{asText(toolUi.strokeOrderUppercaseOnly)}</p>
                )}
              </div>
              <div className="w-full rounded-lg bg-background p-3">
                <h3 className="mb-1 text-sm font-medium text-slate-200">{asText(toolUi.writingTip)}</h3>
                <p className="text-sm text-slate-400">{displayTip}</p>
              </div>
              <button
                type="button"
                onClick={() => speak(resolveLetterName(selectedItem.char, selectedItem.name))}
      ...
      <section className="rounded-xl border border-border bg-surface p-6">
        <h2 id="practice-sheet-heading" className="mb-2 border-b-2 border-blue-500 pb-2 text-lg font-semibold text-slate-100">
          {asText(toolUi.practiceSheetHeading)}
        </h2>
        <p className="mb-4 text-sm text-slate-500">{asText(toolUi.practiceSheetDesc)}</p>
        <div className="mb-4 flex flex-wrap gap-4">
          {(["upper", "lower", "both"] as const).map((mode) => (
            <label key={mode} className="flex cursor-pointer items-center gap-2">
              ...
              />
              <span className="text-sm text-slate-300">{asText(practiceModes[mode])}</span>
            </label>
          ))}
        </div>
