...
const META_PATH = "/tools/language/katakana-study";
...
const KATAKANA_ROWS: [number, number][] = [
  [0, 5],
  [5, 5],
  [10, 5],
  [15, 5],
...
export default function KatakanaStudy() {
  ...
  const handleReplay = useCallback(() => {
    if (displayItem) {
      ...
      const speechText =
        speechLang === "ja-JP"
          ? displayItem.char
          : resolvePronunciation(displayItem.char, displayItem.romaji);
      speak(speechText);
    }
  }, [displayItem, speak, speechLang, resolvePronunciation]);
  ...
  }, [practiceCharSet, resolvePronunciation]);
      ...
      <div className="flex flex-col items-center gap-6 lg:flex-row lg:justify-center lg:items-start">
        <div className="flex-shrink-0 rounded-xl border border-border bg-surface p-6">
          ...
          <div className="space-y-6">
            {KATAKANA_ROWS.map(([start, count], rowIndex) => (
            ...
            <div className="flex flex-col items-center gap-3">
              ...
              <div className="w-full rounded-lg bg-background p-3">
                <h3 className="mb-1 text-sm font-medium text-slate-200">
                  ...
                  <span className="font-normal text-slate-500">({formatStrokeCount(displayItem.strokes)})</span>
                </h3>
                {isHaRow && (
              ...
              <div className="w-full rounded-lg bg-background p-3">
                ...
                <p className="text-sm text-slate-400">
                  {resolveWritingTip(displayItem.char, displayItem.tip)}
                </p>
              </div>
              <button
                type="button"
        ...
        <div className="mb-4 flex flex-wrap gap-4">
          {(["basic", "dakuon", "all"] as const).map((value) => (
            <label key={value} className="flex cursor-pointer items-center gap-2 text-sm text-slate-400">
              ...
              />
              <span>{asText(practiceModes[value])}</span>
            </label>
          ))}
        </div>
