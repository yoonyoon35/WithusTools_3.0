...
const HANGUL_ROWS: [number, number][] = [
  [0, 14],
  [14, 10],
  [24, 5],
  [29, 11],
...
export default function HangulStudy() {
  ...
  const rowLabels = (toolUi.rowLabels as string[]) ?? [];
  const pronunciationNames = asMap(toolUi.pronunciationNames);
  const writingTips = asMap(toolUi.writingTips);
  const speechLang = asText(toolUi.speechLang) || "ko-KR";
  const speechMode = asText(toolUi.speechMode) || "char";
  const practiceModes = asMap(toolUi.practiceModes);

  const resolvePronunciation = useCallback(
    (char: string, fallback: string) => asText(pronunciationNames[char]) || fallback,
    [pronunciationNames]
  );

  const resolveWritingTip = useCallback(
    (char: string, fallback: string) => {
      const tips = asMap(writingTips[char]);
      return asText(tips.tip) || fallback;
    },
    [writingTips]
  );
  ...
  const resolveSpeechText = useCallback(
    (item: HangulItem) =>
      speechMode === "name" ? resolvePronunciation(item.char, item.romaji) : item.char,
    [speechMode, resolvePronunciation]
  );
  ...
  const handleReplay = useCallback(() => {
    if (selectedItem) {
      setReplayKey((k) => k + 1);
      speak(resolveSpeechText(selectedItem));
    }
  }, [selectedItem, speak, resolveSpeechText]);
      ...
      ({ char, romaji }) =>
        `<tr>
          <td class="sample border border-slate-600 p-0 w-[10%] align-middle text-center font-bold bg-slate-100 text-slate-900 relative">
            ...
            <span class="pronunciation absolute right-1 bottom-1 text-sm font-bold text-slate-600">${resolvePronunciation(char, romaji)}</span>
          </td>
  ...
  }, [getPracticeCharList, resolvePronunciation]);
    ...
    <div className="w-full space-y-6">
      <div className="flex flex-col items-center gap-6 lg:flex-row lg:justify-center lg:items-start">
        <div className="flex-shrink-0 rounded-xl border border-border bg-surface p-6">
          ...
          <div className="space-y-6">
            {HANGUL_ROWS.map(([start, count], rowIndex) => (
              <section key={rowIndex}>
                <h3 className="mb-2 text-sm font-semibold text-slate-200">{rowLabels[rowIndex] ?? ""}</h3>
                  ...
                  {HANGUL_DATA.slice(start, start + count).map((item, i) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="text-sm text-slate-500">{resolvePronunciation(item.char, item.romaji)}</span>
                      </li>
                    );
                  })}
                </ul>
        ...
        <div className="sticky top-24 flex-shrink-0 rounded-xl border border-border bg-surface p-6 lg:w-[280px]">
          <h2 className="mb-4 border-b-2 border-blue-500 pb-2 text-lg font-semibold text-slate-100">
            {asText(toolUi.characterDetails)}
          </h2>
          {!selectedItem ? (
            ...
            <div className="flex flex-col items-center gap-3">
              ...
              <div className="text-lg text-slate-500">
                {resolvePronunciation(selectedItem.char, selectedItem.romaji)}
              </div>
              <div className="w-full rounded-lg bg-background p-3">
                <h3 className="mb-1 text-sm font-medium text-slate-200">
                  {asText(toolUi.strokeOrder)}{" "}
                  <span className="font-normal text-slate-500">({formatStrokeCount(selectedItem.strokes)})</span>
                </h3>
                <div
                  key={replayKey}
              ...
              <div className="w-full rounded-lg bg-background p-3">
                ...
                <p className="text-sm text-slate-400">
                  {resolveWritingTip(selectedItem.char, selectedItem.tip)}
                </p>
              </div>
              <button
                type="button"
                onClick={() => speak(resolveSpeechText(selectedItem))}
      ...
      <section className="rounded-xl border border-border bg-surface p-6">
        ...
        <div className="mb-4 flex flex-wrap gap-4">
          {(
            ["all", "consonants", "vowels", "double", "compound"] as const
          ).map((value) => (
