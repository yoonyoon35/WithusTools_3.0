...
const META_PATH = "/tools/pdf/merge-pdf";
...
export default function MergePdfTool() {
  ...
  const addFiles = useCallback(
    (fileList: FileList | null) => {
      ...
      if (arr.length === 0) {
        showMessage(asText(messagesUi.addOnePdf), "error");
        return;
      }
  ...
  const merge = useCallback(async () => {
    if (files.length < 2) {
      showMessage(asText(messagesUi.needTwoPdfs), "error");
      return;
    }
    setLoading(true);
    setPdfUrl(null);
    ...
    try {
      ...
      showMessage(asText(messagesUi.mergeSuccess));
    } catch (err) {
      showMessage(
        err instanceof Error ? err.message : asText(messagesUi.mergeFailed),
        "error"
      );
    } finally {
      setLoading(false);
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-xl border border-border bg-surface p-6">
        ...
        >
          ...
          <p className="mb-2 text-sm text-slate-400">{asText(toolUi.dropHint)}</p>
          <p className="text-xs text-slate-500">{asText(toolUi.supportsMultiple)}</p>
          <input
            ref={fileInputRef}
            type="file"
            accept={ACCEPT_PDF}
        ...
        {files.length > 0 && (
          <div className="mt-4 space-y-4">
                      ...
                      <div className="h-20 w-16 shrink-0 overflow-hidden rounded border border-border bg-slate-900">
                        {previewUrls[i] ? (
                          <iframe
                            ...
                            title={formatUi(asText(toolUi.previewOf), { name: file.name })}
            ...
            <div className="flex flex-wrap items-center gap-4">
              ...
              >
                {loading ? asText(toolUi.merging) : asText(toolUi.mergeButton)}
              </button>
              <button
                onClick={clearAll}
                disabled={loading}
            ...
            {loading && progress && (
              <div className="mt-4 space-y-2">
                <div className="flex items-center justify-between text-sm text-slate-400">
                  <span>
                    {formatUi(asText(toolUi.mergingProgress), {
            ...
            {!loading && (
              <p className="mt-4 text-sm text-slate-500">
                {formatUi(asText(toolUi.filesSelected), {
                  n: files.length,
      ...
      {pdfUrl && (
        <div className="rounded-xl border border-border bg-surface p-6">
          <p className="mb-3 font-semibold text-slate-100">
            {asText(toolUi.mergeSuccessTitle)}
          </p>
